...
4. **Tag (optional)** — `git tag v1.1.1 && git push origin v1.1.1` after the release commit lands.
...
- **[User Guide](docs/USER_GUIDE.md)** — How to use the app: **sidebar** (signed-in chrome, Jakala branding, greeting, navigation), **Account** profile (Slack user ID, Float person link, password), projects, PM/PGM/CAD dashboards (including 1-wk vs 4-wk recovery columns and upcoming PTO/holidays), resourcing (column totals include hidden-from-grid assignees; split-week actuals with keyboard navigation and bulk expand/collapse, Float PTO/holiday hints, collapsible Weekly Actuals to compare Planned vs Float, **Slack resourcing requests** when configured), project **PTO** tab and company **PTO & Holidays** page, budget, Float sync, status reports (**Post to Slack** health updates to the linked **account** channel), and admin (Roles, **Industry groups**, People, Users, **Slack**, **Accounts** — industry inheritance on linked client accounts). Written in standard Markdown for easy copy into Confluence.
- **[Technical Reference](docs/TECHNICAL.md)** — Tech stack, **app shell** (sidebar layout, Jakala branding, greeting row, no layout-level `getDashboardContext`), data model (**IndustryGroup**, account/user links, project inheritance via **Account**), environment variables, week/as-of semantics, split-week actual hours (`ActualHoursMonthSplit`), PTO/holiday persistence and UI, optional **Trigger.dev** scheduled Float sync and **missing-actuals Slack nudges**, **Slack** APIs and schema, permissions, **Projects list page** (filters, pagination, query params, Prisma `select`), scripts (including `migrate:split-week-actuals`), and API overview (e.g. **`/api/projects/[id]`** accepts project **id or slug**, includes **`account.industryGroup`**; **`GET /api/projects/[id]/resourcing`** returns all assignments with **`hiddenFromGrid`** for rollup vs row visibility; admin industry-group routes; status report meeting-notes HTML uses **sanitize-html**). Also Confluence-friendly.
