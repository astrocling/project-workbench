...
4. **Tag (optional)** — `git tag v1.1.0 && git push origin v1.1.0` after the release commit lands.
...
- **[User Guide](docs/USER_GUIDE.md)** — How to use the app: **sidebar** (signed-in chrome, Jakala branding, greeting, navigation), projects, PM/PGM/CAD dashboards (including 1-wk vs 4-wk recovery columns and upcoming PTO/holidays), resourcing (split-week actuals with keyboard navigation and bulk expand/collapse, Float PTO/holiday hints, collapsible Weekly Actuals to compare Planned vs Float, **Slack resourcing requests** when configured), project **PTO** tab and company **PTO & Holidays** page, budget, Float sync, status reports (**Post to Slack** health updates), and admin (Roles, **Industry groups**, People, Users, **Slack**, **Accounts** — industry inheritance on linked client accounts). Written in standard Markdown for easy copy into Confluence.
- **[Technical Reference](docs/TECHNICAL.md)** — Tech stack, **app shell** (sidebar layout, Jakala branding, greeting row, no layout-level `getDashboardContext`), data model (**IndustryGroup**, account/user links, project inheritance via **Account**), environment variables, week/as-of semantics, split-week actual hours (`ActualHoursMonthSplit`), PTO/holiday persistence and UI, optional **Trigger.dev** scheduled Float sync and **missing-actuals Slack nudges**, **Slack** APIs and schema, permissions, **Projects list page** (filters, pagination, query params, Prisma `select`), scripts (including `migrate:split-week-actuals`), and API overview (e.g. **`/api/projects/[id]`** accepts project **id or slug**, includes **`account.industryGroup`**; admin industry-group routes; status report meeting-notes HTML uses **sanitize-html**). Also Confluence-friendly.
