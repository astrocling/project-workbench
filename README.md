...
- **[User Guide](docs/USER_GUIDE.md)** — How to use the app: **sidebar** (signed-in chrome, Jakala branding, greeting, navigation), **Account** profile (Slack user ID, Float person link, password), projects, PM/PGM/CAD dashboards (including 1-wk vs 4-wk recovery columns and upcoming PTO/holidays), resourcing (column totals include hidden-from-grid assignees; split-week actuals with keyboard navigation and bulk expand/collapse, Float PTO/holiday hints, collapsible Weekly Actuals to compare Planned vs Float, **Slack resourcing requests** when configured), project **PTO** tab and company **PTO & Holidays** page, budget, Float sync, status reports (**Post to Slack** health updates), and admin (Roles, **Industry groups**, People, Users, **Slack**, **Accounts** — industry inheritance on linked client accounts). Written in standard Markdown for easy copy into Confluence.
