...
4. **Tag (optional)** — `git tag v1.1.4 && git push origin v1.1.4` after the release commit lands.
...
The same Trigger project can run **missing-actuals Slack nudges** (`trigger/missingActualsNudge.ts`): weekly tasks at **Tuesday / Wednesday / Thursday 15:00 UTC** for the **prior UTC week**. Selection uses the same **Actuals Stale** rules as the Resourcing tab (**Planned > 0**, Actual blank—not **0**—with split-week month-halves; **Float** hours are not used). The worker needs **`SLACK_BOT_TOKEN`**, **`DATABASE_URL`**, and optionally **`WORKBENCH_BASE_URL`**. See [docs/TECHNICAL.md](docs/TECHNICAL.md#missing-actuals-slack-nudges-triggerdev) and the User Guide (*Automated missing-actuals Slack nudges*).
