...
**Slack (optional):** To post **resourcing requests**, **status health updates**, and (with Trigger.dev) **missing-actuals nudges**, set **`SLACK_BOT_TOKEN`** in Vercel (same value as local if you use one bot). Set **`WORKBENCH_BASE_URL`** to your production URL so Slack messages link to the correct host (resourcing and missing-actuals links use **`/projects/{slug}?tab=resourcing`**). Channel IDs are stored in the database: **Admin → Slack** (resourcing channel), **Admin → Accounts** (account channel for **Post to Slack** status updates and Thursday nudges), and project **Settings → Links** (project channel for Tue/Wed missing-actuals nudges only). See [docs/TECHNICAL.md](docs/TECHNICAL.md#slack-integration).
...
4. **Tag (optional)** — `git tag v1.1.2 && git push origin v1.1.2` after the release commit lands.
...
- **[User Guide](docs/USER_GUIDE.md)** — How to use the app: **sidebar** (signed-in chrome, Jakala branding, greeting, navigation), **Account** profile (Slack user ID, Float person link, password), projects, PM/PGM/CAD dashboards (including 1-wk vs 4-wk recovery columns and upcoming PTO/holidays), resourcing (column totals include hidden-from-grid assignees; split-week actuals with keyboard navigation and bulk expand/collapse, Float PTO/holiday hints, collapsible Weekly Actuals to compare Planned vs Float, **Slack resourcing requests** and fulfill-time **variance** replies when configured), project **PTO** tab and company **PTO & Holidays** page, budget, Float sync, status reports (**Post to Slack** health updates to the linked **account** channel), and admin (Roles, **Industry groups**, People, Users, **Slack**, **Accounts** — industry inheritance on linked client accounts). Written in standard Markdown for easy copy into Confluence.
- **[Technical Reference](docs/TECHNICAL.md)** — Tech stack, **app shell** (sidebar layout, Jakala branding, greeting row, no layout-level `getDashboardContext`), data model (**IndustryGroup**, account/user links, project inheritance via **Account**), environment variables, week/as-of semantics, split-week actual hours (`ActualHoursMonthSplit`), PTO/holiday persistence and UI, optional **Trigger.dev** scheduled Float sync and **missing-actuals Slack nudges**, **Slack** APIs and schema (including **resourcing fulfill variance** via **`lib/resourcingFulfillVariance.ts`**), permissions, **Projects list page** (filters, pagination, query params, Prisma `select`), scripts (including `migrate:split-week-actuals`), and API overview (e.g. **`/api/projects/[id]`** accepts project **id or slug**, includes **`account.industryGroup`**; **`GET /api/projects/[id]/resourcing`** returns all assignments with **`hiddenFromGrid`** for rollup vs row visibility; admin industry-group routes; status report meeting-notes HTML uses **sanitize-html**). Also Confluence-friendly.
