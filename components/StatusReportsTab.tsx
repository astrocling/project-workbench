...
const VARIATIONS = [
  ...
  { value: "Modular", label: "Modular (Story Points / Velocity)" },
] as const;

const STORY_POINT_METRICS: { label: string; metric: StoryPointsRow["metric"] }[] = [
  { label: "Planned", metric: "planned" },
  { label: "Completed", metric: "completed" },
  { label: "In Progress", metric: "inProgress" },
  { label: "Carry Over", metric: "carryOver" },
];

const FORM_INPUT_CLASS =
  "block w-full h-9 px-3 rounded-md text-body-sm bg-white dark:bg-dark-raised border border-surface-300 dark:border-dark-muted";

function storyPointRowsForDisplay(
  rows: StoryPointsRow[],
  systemCount: number
): StoryPointsRow[] {
  return STORY_POINT_METRICS.map(({ metric }) => {
    const existing = rows.find((r) => r.metric === metric);
    if (existing) {
      const values = [...existing.values];
      while (values.length < systemCount) values.push(0);
      return { metric, values: values.slice(0, systemCount) };
    }
    return { metric, values: Array(systemCount).fill(0) };
  });
}
          ...
          <div className="bg-white dark:bg-dark-surface rounded-lg border border-surface-200 dark:border-dark-border p-6 space-y-4">
            ...
            <div>
              ...
              <select
                value={formVariation}
                onChange={(e) => {
                  const next = e.target.value as FormVariation;
            ...
            {formVariation === "Modular" && (() => {
              ...
              const storyRows = storyPointRowsForDisplay(
                storyPointPanel?.data.rows ?? [],
                systems.length
              );
                ...
                <div className="space-y-6 rounded-lg border border-surface-200 dark:border-dark-border p-4 bg-surface-50/50 dark:bg-dark-raised/30">
                  <div className="space-y-3">
                    ...
                    {scheduleRows.length === 0 ? (
                      <p className="text-body-sm text-surface-500 dark:text-surface-400">
                        No schedule rows yet. Add a row to define sprint phases.
                      </p>
                    ) : (
                      <div className="overflow-x-auto rounded-md border border-surface-200 dark:border-dark-border">
                        <table className="w-full text-body-sm">
                          <thead>
                            <tr className="border-b border-surface-200 dark:border-dark-border bg-surface-100 dark:bg-dark-raised">
                              <th className="text-left py-2 px-3 font-semibold text-surface-800 dark:text-surface-100">
                                Date range
                              </th>
                              <th className="text-left py-2 px-3 font-semibold text-surface-800 dark:text-surface-100">
                                Label
                              </th>
                              <th className="w-10 py-2 px-2" aria-label="Remove row" />
                            </tr>
                          </thead>
                          <tbody>
                            {scheduleRows.map((row, rowIndex) => (
                              <tr
                                key={rowIndex}
                                className="border-b border-surface-100 dark:border-dark-border last:border-0"
                              >
                                <td className="py-2 px-3">
                                  <input
                                    ...
                                    value={row.dateRange}
                                    onChange={(e) => {
                                      const next = scheduleRows.map((r, i) =>
                                        i === rowIndex ? { ...r, dateRange: e.target.value } : r
                                      );
                                      updateSprintSchedule(next);
                                    }}
                                    placeholder="4/28 – 5/15"
                                ...
                                <td className="py-2 px-3">
                                  <input
                                    ...
                                    placeholder="Development / Internal QA"
                                    className={FORM_INPUT_CLASS}
                                  />
                                </td>
                                  ...
                                  <button
                                    ...
                                    onClick={() =>
                                      updateSprintSchedule(
                                        scheduleRows.filter((_, i) => i !== rowIndex)
                                      )
                                    }
                                    className="inline-flex items-center justify-center h-8 w-8 rounded border border-surface-300 dark:border-dark-muted text-surface-600 dark:text-surface-300 hover:bg-surface-100 dark:hover:bg-dark-border focus:outline-none focus:ring-1 focus:ring-jblue-400"
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center h-8 px-3 rounded text-label-sm border border-jblue-500 text-jblue-600 dark:text-jblue-400 font-medium hover:bg-jblue-50 dark:hover:bg-jblue-950 focus:outline-none focus:ring-1 focus:ring-jblue-400"
                    >
                      Add row
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <p className="text-label-sm font-semibold text-surface-700 dark:text-surface-200 mb-2">
                        Systems
                      </p>
                      <div className="flex flex-wrap gap-2 items-start">
                        {systems.map((sys, sysIndex) => (
                          <div key={sysIndex} className="flex items-center gap-1 min-w-[8rem] max-w-[12rem]">
                            <input
                              type="text"
                              value={sys.name}
                              onChange={(e) => {
                                const nextSystems = systems.map((s, i) =>
                                  i === sysIndex ? { name: e.target.value } : s
                                );
                                updateStoryPointMetrics(nextSystems, storyRows);
                              }}
                              placeholder="System name"
                              className={FORM_INPUT_CLASS}
                              aria-label={`System ${sysIndex + 1} name`}
                            />
                            <button
                              type="button"
                              onClick={() => {
                                const nextSystems = systems.filter((_, i) => i !== sysIndex);
                                const nextRows = storyRows.map((r) => ({
                                  metric: r.metric,
                                  values: r.values.filter((_, i) => i !== sysIndex),
                                }));
                                updateStoryPointMetrics(nextSystems, nextRows);
                              }}
                              className="shrink-0 inline-flex items-center justify-center h-8 w-8 rounded border border-surface-300 dark:border-dark-muted text-surface-600 dark:text-surface-300 hover:bg-surface-100 dark:hover:bg-dark-border"
                              aria-label={`Remove system ${sysIndex + 1}`}
                            >
                              ×
                            </button>
                          </div>
                        ...
                        {systems.length < 4 && (
                          <button
                            type="button"
                            onClick={() => {
                              const nextSystems = [...systems, { name: "" }];
                              const nextRows = storyRows.map((r) => ({
                                metric: r.metric,
                                values: [...r.values, 0],
                              }));
                            ...
                            className="inline-flex items-center justify-center h-8 px-3 rounded text-label-sm border border-jblue-500 text-jblue-600 dark:text-jblue-400 font-medium hover:bg-jblue-50 dark:hover:bg-jblue-950"
                          >
                            Add system
                          </button>
                        )}
                    ...
                    {systems.length > 0 && (
                      <div className="overflow-x-auto rounded-md border border-surface-200 dark:border-dark-border">
                        <table className="w-full text-body-sm">
                          <thead>
                            <tr className="border-b border-surface-200 dark:border-dark-border bg-surface-100 dark:bg-dark-raised">
                              <th className="text-left py-2 px-3 font-semibold text-surface-800 dark:text-surface-100 w-32">
                                Metric
                              </th>
                              {systems.map((sys, sysIndex) => (
                                <th
                                  key={sysIndex}
                                  className="text-right py-2 px-3 font-semibold text-surface-800 dark:text-surface-100 min-w-[5rem]"
                                >
                                  {sys.name.trim() || `System ${sysIndex + 1}`}
                                </th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
                            {storyRows.map((row) => {
                              const metricLabel =
                                STORY_POINT_METRICS.find((m) => m.metric === row.metric)?.label ??
                                row.metric;
                              return (
                                <tr
                                  key={row.metric}
                                  className="border-b border-surface-100 dark:border-dark-border last:border-0"
                                >
                                  <td className="py-2 px-3 font-medium text-surface-700 dark:text-surface-200">
                                    {metricLabel}
                                  </td>
                                  {row.values.map((val, sysIndex) => (
                                    <td key={sysIndex} className="py-2 px-3">
                                      <input
                                        type="number"
                                        min={0}
                                        step={1}
                                        ...
                                        onChange={(e) => {
                                          ...
                                          updateStoryPointMetrics(systems, nextRows);
                                        }}
                                        className={`${FORM_INPUT_CLASS} text-right tabular-nums`}
                                        aria-label={`${metricLabel} for ${systems[sysIndex]?.name || `system ${sysIndex + 1}`}`}
                                      />
                                    </td>
                                  ))}
                                </tr>
                        ...
                        {donutPanels.map((panel) => (
                          ...
                          >
                            <input
                              ...
                              placeholder="Label"
                              className={`${FORM_INPUT_CLASS} flex-1 min-w-[10rem]`}
                            ...
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                max={100}
                                step={1}
                                value={panel.data.manualValue ?? 0}
                                onChange={(e) => {
                                  const num = Number(e.target.value);
                                ...
                                className={`${FORM_INPUT_CLASS} w-20 text-right tabular-nums`}
                                aria-label={`${panel.data.label} percent`}
                              />
                              <span className="text-body-sm text-surface-600 dark:text-surface-300">
                                %
                              </span>
                            </div>
                          </div>
