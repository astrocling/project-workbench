...
- **Project Plan (beta)** — New per-project **Plan** tab: a grid + Gantt schedule builder with phases, nested items (up to 3 levels), types (task, milestone, sign-off, hard deadline, waiting on client, meeting), assumed vs scheduled meetings, business-day durations, project non-working days, and fit/day/week/month zoom. Editors can **drag Gantt bars** (move/resize; calendar-day snap), **drag item rows** (reorder, nest, or move across phases), and **drag phase rows** (reorder phases: drop before another phase, or on **Add phase** to append). Everyone who can open the project sees the tab; editors can change it. Model: `ProjectPlan` / `PlanPhase` / `PlanItem` (+ `PlanNonWorkingDay`). API: `/api/projects/[id]/plan` (+ `plan/phases`, `plan/items`, `plan/non-working-days`), **404** when Plan is off. UI: `components/PlanTab.tsx`, `components/plan/PlanGridGantt.tsx`. The **Timeline** tab is unchanged and still the default source for status reports.

- **Plan — drag to reorder phases** — Phase rows use the same grip as items. Dropping a phase on another phase inserts it **before** that row; dropping on **Add phase** appends it. Items stay in the moved phase. **`PATCH .../plan/phases/[phaseId]`** accepts **`insertBeforePhaseId`** (`string \| null`); destination phases are reindexed `0..n-1` (`reindexPhaseOrders` / `validateInsertBeforePhase` / `resolvePhaseDrop` in **`lib/plan/phaseDrop.ts`**). **`POST .../plan/items`** accepts optional **`insertBeforeItemId`** so new items can be placed among siblings. Tests: `__tests__/lib/plan/phaseDrop.test.ts`.
...
- **Resourcing — HOL/PTO header strip** — Shared header is one unlabeled pill row (no Holiday/PTO labels, cell borders, or card around the strip): **HOL** then **PTO** stack in the week column. Week **scroll chevrons** sit in that same row (sticky left cell), not a toolbar above the pane. The current-week column tint starts at the **date** header and grid bodies, not the pill cells (`.resourcing-shared-header thead tr:first-child th` has no inset wash). Section titles (Planning / Actuals / Float) use `.resourcing-sticky-title` (no bordered gray cell). UI: **`components/ResourcingGrids.tsx`**. CSS: **`app/globals.css`**.

- **Resourcing — PTO/holiday hover weekdays** — Hovering **PTO** or **HOL** (shared header pills and planned-hour cells) lists the UTC weekday for each day as a single letter: **M, T, W, R, F** (**R** = Thursday, so it is distinct from Tuesday **T**). Examples: `Alexandru Cracea: 3 day(s) PTO (M, T, W)`, `Labor Day (F)`. Formatters: `formatPtoHoverLine` / `formatHolidayHoverLine` / `formatWeekdayLetters` in **`lib/ptoDisplayUtils.ts`**. UI: **`components/ResourcingGrids.tsx`**. Tests: **`__tests__/lib/ptoDisplayUtils.test.ts`**.

- **Resourcing — sticky HOL / PTO / week header** — One unlabeled **HOL** / **PTO** pill strip and week-date header is shared by Planned, Actuals, and Float. It stays pinned at the **top of the grid pane** while those tables scroll underneath. **Week columns stay aligned** because the header and all three tables live in a **single** overflow container (`scrollContainerRef`): one `scrollLeft` (chevrons, Shift+wheel, column snap, initial jump to last week). Duplicate week-date rows were removed from Actuals and Float. Title rows and section buttons (**Request Changes**, Actuals chevron, **Sync plan from Float**) remain on each table. CSS: `.resourcing-shared-header` in **`app/globals.css`**. UI: **`components/ResourcingGrids.tsx`**.
...
- **User Guide / Technical Reference / README** — Resourcing shared header: unlabeled stacked **HOL** / **PTO** pills, **scroll chevrons** on that row, current-week tint from the date row down (not the pills), section titles without a gray cell frame. `.resourcing-sticky-title`, `.resourcing-sticky-head-plain`, first-row `box-shadow: none` in **`app/globals.css`**.
...
- **User Guide / Technical Reference / README** — **Total Project** (Budget average rates; was Used (project)). **Resource Efficiency** (was Revenue recovery) on Budget, Overview, and PM/PGM/CAD dashboards (**1-wk / 4-wk efficiency**). API `/revenue-recovery` unchanged.
