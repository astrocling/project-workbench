...
- **Plan tab — blank plan had no way to add an item** — After **Start from blank**, the grid opened in **View**, which hides **Add phase**. Empty phases also had no labeled **Add item** row (only a hover plus on the phase). A plan with no phases now opens in **Edit**. Each expanded phase shows **+ Add item** under its items. Tests: `__tests__/lib/plan/tree.test.ts`.
...
- **Budget Burndown** — Budget and Overview show a week/month chart of period spend plus running **Actual + Plan** and **Plan** lines versus **Contract high**. Missing actuals use planned dollars (not $0). Float hours are not included.
- **In-app User Guide** — Signed-in **User Guide** (sidebar footer) opens the product guide with section navigation: sticky **On this page** on desktop, **Jump to section** on small screens, plus a link to extra Confluence resources. The site footer still links Changelog.
- **Overview — Post week to Slack** — Editors can post a current-week look-ahead to the **project** Slack channel (**Settings → Links**). The control sits on the right of the **Float last updated / Actuals through** header. The message lists people with planned or Float hours this week (Float-only tagged **(Not in Plan)**) and Plan / Timeline items that start or end this week.
- **Project Plan (beta)** — Optional **Plan** tab: a grid plus Gantt for phases and nested items (tasks, milestones, sign-offs, deadlines, waiting on client, meetings), including business-day durations, non-working days, and fit/day/week/month zoom. Everyone who can open the project can see the tab; editors can change it. The **Timeline** tab is unchanged.
  - **Enable per project** — Only Admins see **Settings → Enable Plan tab (beta)**. Saving the toggle shows or hides Plan without a full reload.
  - **Build the schedule** — Drag Gantt bars to move or resize dates. Drag item rows to reorder, nest, or move between phases. Drag phase rows to reorder (drop before another phase, or on **Add phase** to append).
  - **View and Edit** — Compact **View** uses status icons (hover for the full name) so **Not started** / **In progress** are not truncated, and shares space with the Gantt. **Edit** uses the status dropdown and shows the full column set without sideways scroll.
  - **Gantt cues** — A small pad beside kickoff keeps first-day diamonds and bars visible. A red **Today** line sits on the axis (or on the kickoff/end edge if today is outside the plan).
  - **Status reports** — **Standard** and **Milestones** reports can use **Project timeline** or **Project Plan**, with **Phases only** or **Phases + key dates**. Source and density stay locked after create; **Refresh schedule** re-reads Plan the way **Refresh timeline** re-reads Timeline. Set the project default on the Plan tab.

### Changed

- **Budget** — Buffer % is leftover **dollars** after projected burn versus the high dollar budget (Overview, Budget tab, and PM/PGM/CAD tables), not leftover hours versus the hours cap. Projected remaining hours on Budget are leftover dollars ÷ the blended rate (actuals + remaining plan); past vs remaining-plan rates are shown so mix drift is visible. CDA hours-cap remaining is unchanged.
  - **Overview card** — Spend, hours, and average rates sit in an **Overview** card; **Expected remaining** is burn / remaining / buffer only. The three Budget cards share the same title and metric-label treatment.
  - **Labels** — Average rates now say **Total Project** (was Used (project)). **Revenue recovery** is labeled **Resource Efficiency** on Budget, Overview, and PM/PGM/CAD dashboards (portfolio cards, monthly chart, and **1-wk / 4-wk efficiency** columns).
- **Slack post buttons** — Overview **Post week to Slack** and Status Reports **Post to Slack** show a Slack mark on the button.
- **Resourcing — HOL / PTO / week header** — One unlabeled **HOL** then **PTO** pill strip and week dates is shared by Planned, Actuals, and Float. It stays pinned while those tables scroll, and week columns stay aligned because all three grids share one horizontal scroll (chevrons, Shift+wheel, jump to last week). Duplicate week-date rows were removed from Actuals and Float. Section buttons (**Request Changes**, Actuals chevron, **Sync plan from Float**) stay on each table.
  - **Chevrons and tint** — Week scroll chevrons sit in the pill row (sticky left cell), not a toolbar above the pane. Current-week tint starts at the **date** header and the grid bodies, not the pills. Section titles no longer sit in a bordered gray cell.
  - **Hover details** — Hovering **PTO** or **HOL** lists weekday letters **M, T, W, R, F** (**R** = Thursday). Cards grow with the longest line and appear below the trigger so names are not clipped and the sticky header is not torn when scrolling.

### Fixed

- **Navigation — pages opened mid-scroll** — Moving between in-app pages (for example project Budget → **PGM Dashboard**) now starts at the top of the new page. Browser **Back** / **Forward** can still restore the previous scroll position.
- **Status reports — CDA Refresh budget left hours stuck** — After correcting Resourcing actuals, **Refresh budget** on a CDA report updated dollars but left the slide’s hours on the old locked figures. Refresh now rebuilds CDA monthly hours and overall hours actuals as well (milestones unchanged). **Update** alone still does not change hours on the slide.
