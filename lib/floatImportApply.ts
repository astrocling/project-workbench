...
export async function applyFloatImportDatabaseEffects(
  ...
  // Orphaned-pair cleanup: delete ALL FloatScheduledHours rows (any date) for
  // (projectId, personId) pairs that have no ProjectAssignment on the project.
  // This catches rows that aged into the past before the future-only cleanup could remove them.
  if (projectIdsInImport.length > 0) {
    await prisma.$executeRaw`
      DELETE FROM "FloatScheduledHours"
