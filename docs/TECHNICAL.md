...
- **Compact mapping**: When **`scheduleSource === "plan"`**, **`lib/plan/reportSchedule.ts`** (`compactPlanToSchedule`) maps **`ProjectPlan`** phases and dated items into the existing **`snapshot.timeline`** bars/markers shape, stamping **`phaseId` / `itemId`**, honoring **`showOnReports`**, using Plan names (not a Plan-level short label), and marking completed items **`muted`**. **`resolveReportTimelineAxis()`** windows Plan-source to kickoff ∩ lookback/lookahead (not full `Project.startDate`). **`applyTimelineLayout()`** / **`pruneTimelineLayout()`** in `lib/statusReportTimelineLayout.ts` apply per-report overlays at PDF-data read time (`applyTimelineLayoutOverlay`, default true; refresh persist passes false then prunes overlay ids). **`lib/statusReportScheduleBuild.ts`** orchestrates Timeline vs Plan source on create/refresh.
...
- **Create snapshot**: Modular always stores **`period`** / **`today`**. When placed modules need them, create also locks **`budget`**, **`timeline`**, and Plan lists (`modularNeedsBudget` / `modularNeedsTimeline` / `modularNeedsPlanLists`).
