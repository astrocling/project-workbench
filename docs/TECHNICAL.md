...
- **Budget buffer and remaining hours** — `computeBudgetRollups` (`lib/budgetCalculations.ts`) still stores hours-cap leftover as **`remainingAfterProjectedBurnHours*`**. Workbench health uses **dollar** leftover:
  ...
  - **UI:** Budget tab **Overview** card shows spend, hours, and the three rates (**Total Project**, **To date**, **Remaining plan**); **Expected remaining** shows projected burn / remaining $ / remaining hours from dollars / buffer. UI: **`components/BudgetTab.tsx`**. Overview burn donut uses **`burnPercentHighDollars`**. Dashboard **`burnPercent`** is **`burnPercentHighDollars`**. CDA hours-cap math is unchanged.
  - Tests: `__tests__/lib/budgetCalculations.test.ts`.
...
| Project | `/api/projects/[id]/budget` | Budget lines, **`computeBudgetRollups`** payload (including **`bufferPercentHighDollars`**, blended rates, **`remainingHoursFromDollarsHigh`**), last week with actuals, and people summary. **`GET`** cached as **`project-budget`**. UI: **`components/BudgetTab.tsx`**. |
