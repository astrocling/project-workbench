...
6. **Accounts after client re-create** — If a Float client is deleted and re-created under the same name, run sync and confirm **Admin → Accounts** still shows one account for that name with the new Float client id (Slack channel / industry group preserved), and that sync completed without a unique-constraint error.
