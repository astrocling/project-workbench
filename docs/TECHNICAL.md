...
- **Snapshot fields**: **`StatusReportSnapshot.scheduleSource`** (`timeline` \| `plan`, default **`timeline`** when absent) and **`planDensity`** (`phases` \| `phases_and_key_dates`, only when source is **`plan`**). **`resolveScheduleSource()`** / **`resolvePlanDensity()`** in `lib/statusReportPdfData.ts`.
- **Compact mapping**: When **`scheduleSource === "plan"`**, **`lib/plan/reportSchedule.ts`** (`compactPlanToSchedule`) maps **`ProjectPlan`** phases and dated items into the existing **`snapshot.timeline`** bars/markers shape (same **`TimelineBlock`** renderer). **`lib/statusReportScheduleBuild.ts`** orchestrates Timeline vs Plan source on create/refresh (`shouldUseLockedTimeline`, `shouldBuildTimelineFromPlan`, `shouldBuildTimelineFromLegacy`, `buildPlanTimelineCandidate`, `buildLegacyTimeline`).
...
- **API**: **`POST .../status-reports`** — optional **`scheduleSource`** / **`planDensity`** for **Standard** / **Milestones** when Plan is enabled; **400** if Plan source selected but no renderable schedule (`lib/plan/reportScheduleErrors.ts`). **`POST .../refresh-timeline`** rebuilds from Timeline **or** Plan per locked snapshot source; does not change **`scheduleSource`** / **`planDensity`**; returns **400** with **`PLAN_NOT_ENABLED_ERROR`** when the locked source is **`plan`** but Plan has since been disabled, otherwise **`resolveScheduleRebuildError()`** copy (missing project end date vs empty schedule, density-aware for Plan).
- **UI**: `components/StatusReportsTab.tsx` — schedule source + plan density selects on **create** (prefill from **`planReportDefault`** / Plan tab **New status reports use**); read-only on **edit** (still shown for Plan-source reports when **`planEnabled`** is later turned off); **Previous months on schedule** label; **Refresh timeline** vs **Refresh schedule** label/modal copy. Pure form rules in **`lib/statusReportFlags.ts`** (`shouldShowScheduleSourceFields`, `shouldResetScheduleDefaultsOnVariationChange`, `buildScheduleSourceCreatePayload`, etc.). Props from **`ProjectDetailTabs.tsx`**. Tests: `__tests__/lib/statusReportFlags.test.ts`, `__tests__/lib/statusReportPdfData.test.ts`, `__tests__/lib/statusReportScheduleBuild.test.ts`, `__tests__/lib/plan/reportSchedule.test.ts`.
