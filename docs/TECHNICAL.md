...
- **Writes:** For each `(projectId, personId)` still in the merged Float snapshot, all **incomplete** `FloatScheduledHours` rows (`weekStartDate` greater than the sync as-of end-of-previous-week) are **deleted** first, then **non-completed** weeks from the merge are upserted (`!isCompletedWeek` in `lib/weekUtils.ts`). That way weeks that are no longer scheduled in Float do not leave stale hours in the grid. **Completed** weeks are never deleted or overwritten. **`PlannedHours` and `ActualHours` are never written by this path.**
- **Cleanup:** After that, the import deletes **future** `FloatScheduledHours` for any `(projectId, personId)` that appears in the DB for a project in the import but is **not** in the merged snapshot (person removed from the project in Float). Past weeks for that person are never deleted. Project assignments are not removed automatically.
- **Project assignments:** `ProjectAssignment` rows are upserted for every Float `(project_id, people_id)` pair that appears on a task in the sync window, including pairs that produce **no** positive weekly hours — e.g. **zero hours per day** (skipped by `aggregateTasksToWeeklyHours`) or all relevant weekdays excluded (PTO/holidays). Those people still get an assignment when the role resolves to a Workbench role; they may have **no** `FloatScheduledHours` rows. If the Float task/person role cannot be mapped to a Workbench `Role` name (missing `role_id`, unknown Float role name, or name mismatch), sync uses the **first Workbench role by name** as a fallback so the person is still listed on the project (scheduled hours do not depend on role).
...
1. **Resourcing grid** — Sync from Float, then open a project that exists in Float. Confirm **Float** columns show expected hours for a future week you can verify in Float. After **removing or moving** allocations in Float for a person still on the project, run sync again: weeks that should have **no** hours should show **zero** (or empty), not the previous values.
