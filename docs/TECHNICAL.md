...
**Shared sticky header and dual-axis pane:** Holiday, PTO, and week-date rows are a **separate table** (`.resourcing-shared-header`) **above** the three grid cards, with the same `colgroup` / `tableLayout: fixed` / week column widths as the bodies. The header wrapper is `position: sticky; top: 0; z-index: 30` (`.resourcing-shared-header` in `app/globals.css`). Header and bodies share **one** overflow parent (`scrollContainerRef`, `overflow: auto`, `max-height: calc(100dvh - var(--env-banner-height, 0px) - 14rem)`). Horizontal week scroll, column snap, Shift+wheel, chevrons, and initial `scrollLeft` all mutate that element’s `scrollLeft` so week columns cannot drift. Frozen Ready/Person/Role/Total cells still use `position: sticky` + `left` offsets. Actuals and Float **do not** repeat the week-date header row.
