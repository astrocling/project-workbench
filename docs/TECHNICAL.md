...
- **Gating**: **`Project.planEnabled`** (Admin-only **`PATCH`**) controls Plan tab visibility and Plan API access. Per-project flag replaces legacy env allowlists (`lib/plan/feature.ts` → **`isPlanTabEnabled`**). **Plan is off for every project** until an Admin enables it.
