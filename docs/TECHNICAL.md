...
- **Route:** **`/user-guide`** (`app/(app)/user-guide/page.tsx`) — signed-in only. Renders **`docs/USER_GUIDE.md`** via **`MarkdownDoc`** (`react-markdown` + `remark-gfm`, GitHub-style heading ids in **`lib/markdownHeadings.ts`**). H2 chapters drive a sticky **On this page** TOC (desktop) and **Jump to section** select (mobile). Banner links to additional Confluence resources (`CONFLUENCE_USER_GUIDE_URL` in **`lib/userGuide.ts`**). Footer **User Guide** is in **`AppFooter`**. Changelog uses the same **`MarkdownDoc`**.

### Syncing Confluence (maintainers)

The in-app page is the product copy. To update the [Confluence User Guide folder](https://ffwagency.atlassian.net/wiki/spaces/PW/folder/6305710084/User+Guide) from the same markdown, install the [Atlassian Rovo MCP](https://developer.atlassian.com/cloud/rovo-mcp/guides/getting-started/) in Cursor ([marketplace plugin](https://cursor.com/marketplace/atlassian)), complete OAuth for `ffwagency.atlassian.net`, and ask the agent to sync from **`docs/USER_GUIDE.md`**. This is editor-side only; the Next.js app does not call Confluence.
...
| **Project** | Project (slug, name, client, start/end dates, status, optional **`accountId`** → effective industry group comes from **`Account.industryGroup`** when linked, optional **`slackChannelId`** for Wed/Tue missing-actuals nudges and weekly look-ahead, optional single rate, notes, SOW/estimate/float/metric links, resourcing thresholds, `cdaEnabled`, `cdaReportHoursOnly`, **`planEnabled`** (User/Admin **PATCH**; gates Plan tab and Plan APIs), **`planReportDefault`** (`timeline` \| `plan`; default schedule source for new Standard/Milestones reports), optional clientSponsor/keyStaffName for status reports). |
...
| **ProjectPlan** / **PlanPhase** / **PlanItem** / **PlanNonWorkingDay** | Optional project **Plan** tab (gated by **`Project.planEnabled`**). Phases hold nested items (task, milestone, sign-off, deadline, waiting on client, meeting). Meeting fields live on **`PlanItem`** — there is no `PlanMeeting` table. |
...
- **`revalidateProjectDetail()`** (`lib/projectCache.ts`) is the single entry point for invalidating the **`project-detail`** tag that **`getCachedProjectBySlugOrId`** applies (30s TTL, otherwise unchanged). It revalidates with **`IMMEDIATE_EXPIRATION`** (`{ expire: 0 }`, `lib/cacheProfiles.ts`) instead of the `"max"` profile: `"max"` only marks the tag **stale**, so the next read is served the pre-write row under stale-while-revalidate. **Project Settings** re-reads those server props via `router.refresh()` immediately after saving (that stale read previously fed a compensating **`PATCH`** that undid the **Plan** toggle), and a deleted project must not be served from cache either. **`updateTag`** is not an option here — it throws outside Server Actions.
- **Call sites:** **`PATCH`** / **`DELETE`** `/api/projects/[id]`, **`POST /api/projects`** (when Float backfill matched), and both save paths of **`PATCH /api/projects/[id]/actual-hours`**. The **`portfolio-metrics`**, **`projects-list`**, and **`project-resourcing*`** tags keep the `"max"` profile.
- **Settings autosave and the Plan nav:** `components/ProjectSettingsTab.tsx` calls `router.refresh()` after a save that changed **`planEnabled`** so the tab list re-renders. Autosave does not abort a **`PATCH`** in flight, so the refresh is **deferred** whenever the form moved on while that save ran (`resolvePlanNavRefresh` in `lib/projectSettingsForm.ts`, against the newest payload held in a ref) and runs after the follow-up save settles — or as soon as the form is clean again, if the edit was undone instead of saved. Arriving server props are also **not** applied while the form is dirty (`shouldHydrateServerProps`), so no refresh can overwrite unsaved edits. Tests: `__tests__/lib/projectSettingsForm.test.ts`.
- **Tradeoff:** the tag is global, so any of these mutations expires **every** project's cached detail payload and the next read of each costs one DB round-trip. The `"max"` call already marked all of them stale and revalidated on read; the change is that reads now block rather than serve stale. `actual-hours` is the hottest of these paths and pays that cost for correctness. Per-project tags (`project-detail:{id}`) are the precise follow-up.
...
- **`lib/portfolioMetrics.ts`** — Builds per-role portfolio metrics and `projectTableRows` for the dashboard projects table. Each row includes **`recoveryThisWeekPercent`** (resource efficiency % for the most recent completed week only—aligned with `revenueRecovery.thisWeek`) and **`recovery4WeekPercent`** (rolling sum over the previous four completed weeks—aligned with the “Previous 4 weeks” portfolio card). **`burnPercent`** / **`bufferPercent`** are **dollar** (`burnPercentHighDollars` / `bufferPercentHighDollars`). Also: actuals status, status-report RAG / stale flag, and **`requestOpen`** (true when any `ReadyForFloatUpdate` has `ready: true` for a person on a non–hidden-from-grid assignment—same visibility rule as `GET /api/projects/[id]/resourcing`).
...
- **Budget buffer and remaining hours** — `computeBudgetRollups` (`lib/budgetCalculations.ts`) still stores hours-cap leftover as **`remainingAfterProjectedBurnHours*`**. Workbench health uses **dollar** leftover:

  - **`bufferPercentHighDollars`** = remaining projected-burn dollars ÷ high dollar budget. Project Overview **Buffer % ($)** and dashboard **`bufferPercent`** use this (not leftover hours).
  - **`remainingHoursFromDollarsHigh`** = remaining dollars ÷ **`blendedRateForRemainingHours`**. Blend is hours-weighted person rates: actuals on completed weeks + planned on current/future (**`blendedRateProject`**, same mix as projected burn). Fallbacks: **`blendedRatePast`**, then **`blendedRateFuture`**, then implied contract `high$ / highHours`. Helpers: **`blendedRateFromTotals`**, **`resolveBlendedRateForRemainingHours`**.
  - **UI:** Budget tab **Overview** card shows spend, hours, and the three rates (**Total Project**, **To date**, **Remaining plan**); **Expected remaining** shows projected burn / remaining $ / remaining hours from dollars / buffer.
  - **Budget Burndown** — `computeBudgetBurndownSeries` (`lib/budgetCalculations.ts`) builds weekly points (Monday spine from `getAllWeeks`) then month rollups by **week-start** `YYYY-MM`. Period $ = actuals on completed weeks with full actuals, else planned $ (missing actuals are not $0). **Actual + Plan** cumulative uses `actualHours ?? plannedHours` × rate (same as **`projectedBurnDollars`**). **Plan** cumulative is planned $ only. Contract high = sum of `BudgetLine.highDollars`. Returned on **`GET /api/projects/[id]/budget`** as **`burndown`** and on project-page `initialBudgetData`. UI: **`BudgetBurndownChart.tsx`** (Recharts composed chart; Week | Month toggle, default Week; legend Actual + Plan, Plan, Period burn; contract-high label top-left). Compact copy on project Overview. Float excluded. Tests: `__tests__/lib/budgetCalculations.test.ts`.
  - Overview burn donut uses **`burnPercentHighDollars`**. Dashboard **`burnPercent`** is **`burnPercentHighDollars`**. CDA hours-cap math is unchanged.
- **Resource Efficiency (UI label)** — Display name for plan-vs-actual dollar recovery (`lib/revenueRecovery.ts`, `GET /api/projects/[id]/revenue-recovery`, portfolio `revenueRecovery`). User-facing copy is **Resource Efficiency** (Budget section, Overview pies, dashboard **Portfolio resource efficiency** cards, **1-wk / 4-wk efficiency** columns). Code, cache tags, and API paths still use `revenue-recovery` / `recoveryPercent`. UI: **`RevenueRecoveryCard.tsx`**, **`RevenueRecoveryShared.tsx`**, **`DashboardProjectsTable.tsx`**, dashboard pages.
- **`components/DashboardProjectsTable.tsx`** — Renders the sortable table (including the **Request** column). Recovery columns are labeled **1-wk efficiency** and **4-wk efficiency**. Sort state is driven by URL query params `sort` and `dir` on `/pm-dashboard`, `/pgm-dashboard`, and `/cad-dashboard` (see `app/(app)/*-dashboard/page.tsx`). Valid `sort` keys include `requestOpen`.
...
Scheduled hours are loaded from the **Float API** (not file upload). Orchestration: `lib/float/syncFloatImport.ts` (`executeFloatApiSync`) → account reconcile → `applyFloatImportDatabaseEffects` in `lib/floatImportApply.ts`. The sync also calls `/v3/timeoffs`, `/v3/public-holidays`, and `/v3/holidays` (team holidays; filtered client-side to the sync window) for the same date window as tasks; `lib/float/excludedDays.ts` merges **full-day time off** (per person) and **regional** public/team holidays into `excludedUtcDatesByFloatPeopleId`, and `lib/float/taskAggregation.ts` (`aggregateTasksToWeeklyHours`) subtracts those UTC weekdays before writing `FloatScheduledHours`. **Partial time off** (under 8 hours per day, and not `full_day`) is **not** added to that exclusion set, so remaining Float task hours on those days still count. **Multiple Float tasks** for the same `(project_id, people_id)` are **summed** (per UTC day, then into Monday weeks)—including overlapping workstream blocks and split days in the same week. Duplicate API rows for the same `task_id` are collapsed first so pagination overlap is not double-counted. **Time off → person mapping** must match Float’s API: `buildExcludedUtcDatesByFloatPeopleId` uses **`people_ids`** (array) when present, else **`people_id`**—the same resolution as `lib/float/ptoholidaySyncWriters.ts` for `PTOHolidayImpact`—so scheduled-hour totals stay consistent with PTO UI when Float omits top-level `people_id`. The same sync run **persists** day-level rows into **`PTOHolidayImpact`** for UI features (Resourcing pills, **PTO** tab, **`/pto-holidays`**, dashboard widget). Admin route: `GET`/`POST` `app/api/admin/float-sync/route.ts` (UI: **Admin → Float sync**, `/admin/float-sync`; `/admin/float-import` redirects there). **Admin → Holidays** uses `GET /api/admin/float-holidays` (read-only JSON tables).
...
2. **PTO vs Float hours** — For a future week where someone has **full-day time off** in Float, confirm the **Float** column total is **reduced** on those weekdays (not full allocation as if they were working), and that the **PTO** tab / pills agree after sync. For **partial PTO** with remaining project hours in Float, the **Float** column should still show those remaining hours (not 0).
...
Optional **background** runs use the same core pipeline as `POST /api/admin/float-sync`: `trigger/floatSync.ts` defines two **`schedules.task`** jobs (`float-sync-weekday`, `float-sync-weekend`) that call `executeFloatApiSync` with `uploadedByUserId: null`. Config: `trigger.config.ts` (`dirs: ["./trigger"]`, **`runtime: "node-24"`**, `maxDuration` for long runs). **Environment** for the Trigger.dev worker (project dashboard / deploy target) must include **`DATABASE_URL`**, **`FLOAT_API_TOKEN`**, and optionally **`FLOAT_API_USER_AGENT_EMAIL`**—same as the app for Float calls. These tasks **do not** invoke Next.js `revalidateTag` (no request context); interactive admin sync still handles cache invalidation for resourcing tags. Package: `@trigger.dev/sdk` (see `package.json`).

**Runtime and deploy (independent of Vercel):** Keep **`runtime: "node-24"`**. If that field is omitted, Trigger.dev defaults to Node **21.7.3**; **new** Trigger deploys using Node 21 fail after **5 October 2026** (existing workers keep running until you redeploy). A **Vercel** production or preview deploy does **not** update Trigger workers, and you do **not** wait for an app release to pick up a runtime change. After changing `trigger.config.ts` or files under `trigger/`, deploy workers with a CLI version that **matches** `@trigger.dev/sdk` (example: SDK `4.5.12` → `npx trigger.dev@4.5.12 deploy`). `npx trigger.dev@latest deploy` aborts when the latest CLI and the installed SDK differ. Confirm this project is off the Node 21 list with `npx trigger.dev@latest projects list --needs-update`. Local task runner: `npx trigger.dev@<sdk-version> dev`. See [Trigger.dev config — runtime](https://trigger.dev/docs/config/config-file#runtime).
...
| **Slack user ID** | **Admin → Users** → **`User.slackUserId`** | Mentions (health update, weekly look-ahead team list), Tuesday PM DMs, requester display in Slack |
| **Person ↔ User link** | **`Person.userId`** (Prisma) | Resolve PM/PGM **`Person`** rows to **`User.slackUserId`** for mentions and DMs |
| **Project channel** | **Settings → Links** → **`Project.slackChannelId`** (`PATCH /api/projects/[id]`) | Tue fallback, Wed nudge, weekly look-ahead digest |
...
**Channel resolution for weekly look-ahead:** `POST .../slack/weekly-lookahead` calls **`resolveProjectSlackChannel`** in **`lib/slackChannels.ts`**, which requires a non-empty **`Project.slackChannelId`**. It does **not** read the account channel. Digest assembly lives in **`lib/slack/weeklyLookahead.ts`**: current UTC Monday–Sunday week; people with **PlannedHours** or **FloatScheduledHours** `> 0` that week (Float-only tagged **(Not in Plan)**). **`formatPersonLine`** appends weekly hours (**Planned** when `> 0`, else **Float**). Plan items and Timeline markers/bars whose **start date or end date** falls in that week (not full-range overlap). **`formatDatedItemLine`** prints a same-day date, an in-week start–end range, or **starts (through …)** / **ends (from …)** when the other bound is outside the posted week. Tests: `__tests__/lib/slackChannels.test.ts`, `__tests__/lib/slack/weeklyLookahead.test.ts`.

**UI:** Overview **Post week to Slack** is in **`ProjectDetailTabs.tsx`**, right-aligned on the **Float last updated / Actuals through** header (editors only; disabled without a project channel). Status Reports **Post to Slack** is in **`StatusReportsTab.tsx`** (list control + confirm in the note dialog). Both controls use the monochrome Slack mark in **`components/SlackIcon.tsx`**.
...
| `POST /api/projects/[id]/slack/weekly-lookahead` | Session (User/Admin) | Post current-week team (hours via **`formatPersonLine`**) + Plan/Timeline dates (starts/ends/range via **`formatDatedItemLine`**) to the **project** channel (`Project.slackChannelId`). Overview **Post week to Slack** (`ProjectDetailTabs.tsx`). Empty POST body. |
...
| **Weekly look-ahead** (`POST .../slack/weekly-lookahead`) | `{WORKBENCH_BASE_URL}/projects/{slug}?tab=overview` | — |

**URL builder:** `lib/workbenchUrls.ts` — `getWorkbenchBaseUrl()`, `projectTabUrl(slug, tab)`, `projectResourcingUrl(slug)`, `projectStatusReportsUrl(slug)`. Used by Slack message routes so links stay aligned with in-app tab routing. Weekly look-ahead uses `projectTabUrl(slug, "overview")`.
...
| `npx trigger.dev@<sdk-version> deploy` | Optional: deploy Trigger.dev tasks (`trigger/`): scheduled Float sync, missing-actuals Slack nudges. Pin **`<sdk-version>`** to `@trigger.dev/sdk` in `package.json` / `node_modules`. Keep **`runtime: "node-24"`**. Independent of Vercel. Env: **`DATABASE_URL`**, **`FLOAT_API_TOKEN`**, **`SLACK_BOT_TOKEN`** as applicable. List outdated runtimes: `npx trigger.dev@latest projects list --needs-update`. |
...
| Project | `GET/PATCH/DELETE /api/projects/[id]` | Single project CRUD. **`[id]`** is either the project’s **primary key (CUID)** or its current **slug** (`resolveProject` in `app/api/projects/[id]/route.ts`). Responses include **`account`** when present (**`account.industryGroup`** for inherited industry taxonomy). **`PATCH`** accepts optional **`slackChannelId`** (Slack channel `C…` or null) for **missing-actuals Tue/Wed nudges** and **weekly look-ahead** (not status health posts). **`PATCH`** and **`DELETE`** revalidate Next.js cache tags **`portfolio-metrics`**, **`projects-list`**, and **`project-detail`** (the project detail page uses `getCachedProjectBySlugOrId` in `lib/projectCache.ts` with a `project-detail` tag). Both use **`revalidateProjectDetail()`** for that tag, see *Project detail cache invalidation* below. **`PATCH`** accepts optional `cdaReportHoursOnly` (boolean): when `true`, CDA “Overall” status copy and CDA status reports omit budget-dollar columns (hours columns only). **`PATCH`** accepts optional **`planEnabled`** (User or Admin) and **`planReportDefault`** (`timeline` \| `plan`). See *Plan schedule on status reports* below. **Client note:** `components/ProjectSettingsTab.tsx` calls **`PATCH`**, **backfill-float**, and **sync-plan-from-float** with **`/api/projects/{projectId}`** (CUID) so autosave still resolves the row after a **name** change regenerates the slug; on **`PATCH`** success, if the response **`slug`** differs from the URL segment, the app **`router.replace`**s to `/projects/{newSlug}?tab=settings`. |
| Project | `POST /api/projects/[id]/slack/health-update`, `POST .../slack/weekly-lookahead`, `POST .../slack/resourcing-request`, `POST .../slack/resourcing-request/fulfill` | Slack integrations; see [Slack integration](#slack-integration). |
...
| Project | `/api/projects/[id]/budget` | Budget lines, **`computeBudgetRollups`** payload (including **`bufferPercentHighDollars`**, blended rates, **`remainingHoursFromDollarsHigh`**), **`burndown`** (`computeBudgetBurndownSeries`: weekly + monthly period/cumulative $), last week with actuals, and people summary. **`GET`** cached as **`project-budget`**. UI: **`components/BudgetTab.tsx`**, **`BudgetBurndownChart.tsx`**. |
| Project | `/api/projects/[id]/rates` | Role rates for the project. |
| Project | `/api/projects/[id]/revenue-recovery` | Resource efficiency (plan vs actual dollars): to date, recent weeks, monthly. UI label **Resource Efficiency**; route name unchanged. |
...
| Project | `/api/projects/[id]/plan`, `plan/phases`, `plan/phases/[phaseId]`, `plan/items`, `plan/items/[itemId]`, `plan/non-working-days` | Plan tab CRUD (**404** when **`project.planEnabled`** is false). Gated by **`requirePlanSessionForProject`** / **`requirePlanEditSessionForProject`** (`lib/plan/feature.ts`, built on **`isPlanTabEnabled`** there and **`requireSession`** / **`canEdit`** in `lib/plan/api.ts`). Item payload rules (point vs range dates, nesting depth, cycles) live in **`lib/plan/itemRules.ts`** (`normalizeItemDates`, `validateItemPayload`). **`PlanMeetingStatus`** is **`unscheduled`** \| **`scheduled`** (renamed from **`assumed`** in `20260914180000_plan_meeting_unscheduled`). Range vs point for meetings follows that flag; **Complete** is **`PlanItemStatus`**. **`PATCH .../plan/items/[itemId]`** coerces inverted start/end via **`coercePlanItemDates`** (`lib/plan/dateInput.ts`: moving start keeps duration; moving end before start clamps end up). Accepts optional **`phaseId`** (descendants move with the item) and **`insertBeforeItemId`** (`string \| null`): when that key is present, destination siblings are reindexed `0..n-1` (`reindexSiblingOrders` / `validateInsertBefore` in **`lib/plan/itemDrop.ts`**; `null` appends). **`POST .../plan/items`** accepts the same optional **`insertBeforeItemId`** after create. Item drop placement is resolved in the UI by **`resolveItemDrop`**. **`PATCH .../plan/phases/[phaseId]`** accepts optional **`insertBeforePhaseId`** (`string \| null`): when that key is present, all phases on the plan are reindexed `0..n-1` (`reindexPhaseOrders` / `validateInsertBeforePhase` in **`lib/plan/phaseDrop.ts`**; `null` appends). Phase drop placement is **`resolvePhaseDrop`** (no-op if already last and appending, or already immediately before the target). Gantt move/resize uses **`ymdAtPercent`** / **`applyGanttDrag`** in **`lib/plan/positioning.ts`** (calendar-day snap; date-only PATCH on pointerup). UI: **`components/PlanTab.tsx`** and **`components/plan/PlanGridGantt.tsx`**; tab visibility in **`ProjectDetailTabs.tsx`**. Date cells (`DateCell` in `PlanGridGantt.tsx`, decided by **`resolvePlanDateCellEdit`**) save a complete, changed date exactly once; **`patchItemDates`** sends the coerced start/end pair. **`lastSubmitted`** is rolled back if the PATCH fails. New items in an empty phase default to **today** (clamped to the plan); otherwise the phase’s earliest start (`defaultNewItemDate`). End date inputs use **`min`** = start. Phase complete is derived (`isPhaseComplete` in `lib/plan/completion.ts`) when every item is complete. |
| Project | `/api/projects/[id]/status-reports`, `status-reports/[reportId]`, `status-reports/[reportId]/pdf`, **`POST status-reports/[reportId]/refresh-timeline`**, **`POST status-reports/[reportId]/refresh-budget`**, **`POST status-reports/[reportId]/refresh-cda-milestones`**, **`POST status-reports/[reportId]/refresh-plan-lists`** | Status reports CRUD and PDF export (PDF may be cached in Vercel Blob). **`POST`** (create) accepts optional **`showBudget`** (boolean, default **`true`**) for **Standard** variation; stored in snapshot. **`POST`** (create) for **Standard** / **Milestones** / **Modular** accepts optional **`scheduleSource`** (`timeline` \| `plan`) and **`planDensity`** when Plan is enabled—see *Plan schedule on status reports* below. **`POST`** / **`PATCH`** accept optional **`panels`** (v1 `{ version, layout, modules }` or legacy array; **`normalizeModularPanels`**) for **Modular** variation; persisted on **`StatusReport.panels`**. **`PATCH`** accepts **`showBudget`** and merges it into the report snapshot. **`POST`** (create) returns **400** if the project has **missing actuals** (completed weeks with planned hours but no actuals)—`projectHasMissingActuals` (`lib/projectActualsStale.ts`)—**except** when **`variation === "Modular"`** (API bypass; the Status Reports tab form still blocks create client-side when actuals are stale). The client stale gate reads **`rollups.missingActuals`** seeded from **`initialBudgetData`**; **`ProjectDetailTabs.tsx`** keeps that payload in state (`budgetData`) and refreshes it in **`refetchBudgetStatus`** (called by **`onActualsUpdated`** after each actuals save), so opening Status Reports after updating actuals clears the stale banner and re-enables **New report** immediately without waiting for the `project-budget` / `project-detail` caches to expire. **`POST refresh-timeline`** (session, User/Admin): merges a freshly built **timeline** into the existing snapshot from live **`TimelineBar`** / **`TimelineMarker`** rows **or** compact Plan mapping per locked **`snapshot.scheduleSource`** (`rebuildTimelineFromProject` in `buildStatusReportPdfData`; requires valid snapshot). **`POST refresh-budget`** (session, User/Admin; not Modular): recomputes **`snapshot.budget`** from live budget lines + actuals (`rebuildBudgetFromProject`); on **CDA** also replaces **`snapshot.cda`** monthly rows / **`totalMtdActuals`** / **`overallBudget`** while preserving milestones (`shouldRebuildCdaBudgetFromProject`, `applyCdaBudgetRefresh`). **`POST refresh-cda-milestones`** (session, User/Admin, **`variation === "CDA"`** only): replaces **`snapshot.cda.milestones`** with **`buildCdaMilestonesFromProject(project.cdaMilestones)`**; other **`snapshot.cda`** fields unchanged. **`POST status-reports/[reportId]/refresh-plan-lists`**: Modular only when a Plan list module is placed; merges live Plan meetings/activities into the snapshot. Refresh routes call **`deleteCachedPdf(reportId)`** and **`revalidateTag(`status-report-${reportId}`)`**. Routes: `refresh-timeline/route.ts`, `refresh-budget/route.ts`, `refresh-cda-milestones/route.ts`, `refresh-plan-lists/route.ts`. |
...
- **`ptoHolidayByWeek`** — map of week start key (`YYYY-MM-DD` Monday) → day-level PTO/holiday entries for **all** assignees. Each entry: `{ personId, type: "PTO" | "HOLIDAY", date: "YYYY-MM-DD", hours, label, isPartial }`. Used by the **shared** Resourcing **HOL** / **PTO** header pills (one header for Planned, Actuals, and Float), planned-cell borders, and hover cards (`components/ResourcingGrids.tsx`). Weekend **HOLIDAY** rows are omitted from this payload (`isUtcWeekdayDate`).
...
**Resourcing grid UI:** The column for the UTC week where `isCurrentWeek()` is true (`lib/weekUtils.ts`, same Monday 00:00 UTC week boundaries as the rest of the app) gets a subtle inset tint via the `resourcing-current-week` class on `th`/`td`, styled in `app/globals.css`. `ResourcingGrids` applies it to the **week-date** header, body cells, and total/variance rows — **not** the HOL/PTO pill row. `.resourcing-shared-header thead tr:first-child th { box-shadow: none }` keeps that strip clear if the class is ever present.

**Shared sticky header and dual-axis pane:** Holiday/PTO pills and week-date rows are a **separate table** (`.resourcing-shared-header`) **above** the three grid cards, with the same `colgroup` / `tableLayout: fixed` / week column widths as the bodies. Pills share one unlabeled row (reserved **HOL** slot then **PTO** slot per week; no row labels, cell borders, or outer card). When week columns overflow, **Scroll left / right** buttons render in the sticky left cell of that pill row (`scrollState.scrollable`). The header wrapper is `position: sticky; top: 0; z-index: 30` (`.resourcing-shared-header` in `app/globals.css`). Header and bodies share **one** overflow parent (`scrollContainerRef`, `overflow: auto`, `max-height: calc(100dvh - var(--env-banner-height, 0px) - 14rem)`). Horizontal week scroll, column snap, Shift+wheel, chevrons, and initial `scrollLeft` all mutate that element’s `scrollLeft` so week columns cannot drift. Frozen Ready/Person/Role/Total cells still use `position: sticky` + `left` offsets. Actuals and Float **do not** repeat the week-date header row. Section titles use `.resourcing-sticky-title` (opaque, no inset cell frame). The pill-row sticky gutter uses `.resourcing-sticky-head-plain` (header background, no inset frame).

**PTO/holiday hover cards:** `ResourcingGrids` formats hover copy with **`lib/ptoDisplayUtils.ts`** (`formatPtoHoverLine`, `formatHolidayHoverLine`, `formatWeekdayLetters`). Weekday letters are UTC **`S M T W R F S`** with **Thursday = R** (distinct from Tuesday **T**). Cards use `w-max` / `whitespace-nowrap` (no short max-width). They are **`position: fixed` portals** (`hoverTip` + `createPortal` to `document.body`, class `PTO_HOLIDAY_PORTAL_BOX`) anchored below the trigger’s `getBoundingClientRect()`, not in-cell `top-full` overlays. Grid cells stay `overflow-hidden` so sticky paint is not broken. `setHoverTip(null)` runs on pane scroll. Tests: `__tests__/lib/ptoDisplayUtils.test.ts`.
...
- **Snapshot and PDF data**: `lib/statusReportPdfData.ts` builds `StatusReportPDFData` for view, share, and preview. On create, `POST .../status-reports` persists a JSON **snapshot** (period, `today`, budget, CDA—including **`cda.milestones`**—timeline, `timelinePreviousMonths`, `showBudget`, **`scheduleSource`**, **`planDensity`**, etc.). Reads prefer snapshot fields so project edits do not alter old reports. **`rebuildTimelineFromProject`** in `BuildStatusReportPdfDataOptions` skips `snapshot.timeline` for one build (used by **`POST .../refresh-timeline`**). **`rebuildBudgetFromProject`** skips `snapshot.budget` for one build (used by **`POST .../refresh-budget`**); on **CDA** it also rebuilds locked monthly hours / **`totalMtdActuals`** / **`overallBudget`** via **`shouldRebuildCdaBudgetFromProject`** + **`applyCdaBudgetRefresh`** (milestones preserved). **`rebuildCdaMilestonesFromProject`** skips `snapshot.cda.milestones` for one build (optional; refresh route writes snapshot directly). **`buildCdaMilestonesFromProject()`** maps live **`CdaMilestone`** rows to sorted snapshot milestone objects (ISO date strings via **`toISOString().slice(0, 10)`**). UI: `components/StatusReportsTab.tsx` (**Refresh timeline** / **Refresh schedule** / **Refresh budget** / **Refresh milestones on report** + confirmation modals); preview refetch via **`dataRefreshKey`** on `components/StatusReportPreview.tsx`.
...
- **Plan PDF export**: **Download PDF** on the Plan toolbar (`components/PlanTab.tsx` / `PlanGridGantt`) captures designed print pages in `components/plan/PlanPrintDocument.tsx` via `capturePlanToPdf()` (`lib/planPdfCapture.ts`, html2canvas-pro + jsPDF). Pages are **US Letter landscape** (`792×612` pt). Page 1 is metrics + a Fit-to-page Gantt (`printGanttColWidth` always fills the axis). Page 2 is phase detail and assumptions. This is **not** a screenshot of the interactive grid (that produced oversized files). No server `@react-pdf` fallback. Label contrast: `lib/plan/ganttBarLabel.ts`. Tests: `__tests__/lib/plan/planPdfCapture.test.ts`, `__tests__/lib/plan/ganttBarLabel.test.ts`, `__tests__/lib/plan/printLayout.test.ts`.
...
- **Detailed Plan PDF page**: When **`snapshot.includeDetailedPlan`** is true, preview renders **`PlanPrintDocument`** (`chartOnly`) under the slide and client capture adds a US Letter landscape page (`PLAN_PDF_PAGE_W_PT` × `PLAN_PDF_PAGE_H_PT`) from `[data-plan-print-page=chart]`.
- **Timeline layout (tab and status report)**: The project Timeline tab and **Project timeline** status reports share week-proportional month columns (`getWeeksInMonthsForRange()` in `lib/monthUtils.ts`, used by `TimelineTab.tsx`, `StatusReportView.tsx`, and `StatusReportDocument.tsx`). The Timeline tab uses a larger row height (52px). Status-report **row height depends on schedule source** — see *Timeline on status reports* below.
- **Timeline on status reports (`TimelineBlock`)**: Implemented in **`components/StatusReportView.tsx`** (HTML preview + client PDF capture) and **`components/pdf/StatusReportDocument.tsx`** (server `@react-pdf/renderer` fallback). **Apply visual changes to both files** so preview, client export, and server PDF stay aligned.
  - **Layout mode**: **`getStatusReportTimelineMetrics(scheduleSource)`** in **`lib/statusReportTimelineLayout.ts`**. **`StatusReportPDFData.scheduleSource`** is set in **`buildStatusReportPdfData`** from **`resolveScheduleSource(snapshot)`** (default **`timeline`** when the snapshot omits it). Both `TimelineBlock`s pass that field through.
    - **`timeline`** (and omitted / legacy): **overlay** — **14px** rows, bars fill the row (`top`/`bottom` padding), markers centered on the date. This is the pre-Plan slide layout. Four overlay rows fit **`TIMELINE_SLOT_HEIGHT = 70`**.
    - **`plan`**: **lanes** — full-width chart (no left name column). Bars use Plan phase colors with the phase name inside the bar (contrast from `ganttBarLabelTextColor`). No key-date pins. Display axis is always re-resolved from Plan kickoff / first work + lookback/lookahead (even for locked snapshots that stored a full project year). An Arrange window can only **narrow** that range, not reopen Jan–Dec. Month headers use short names when more than four months are visible.
  - **Regression**: Shipping Plan band metrics as the *only* `TimelineBlock` size made **Project timeline** reports (typically four rows) overflow the 16:9 slide and paint over the activity columns. Tests: **`__tests__/lib/statusReportTimelineLayout.test.ts`**.
  - **Pinned layout (PDF)**: `TIMELINE_SLOT_HEIGHT = 70` reserves vertical space for the timeline above the budget block (`BUDGET_BLOCK_RESERVED = 70` unchanged). Worst-case sizing assumes report-date label row + month header + up to four **14px** overlay rows + buffer (Plan-source slides use up to four **18px** named lanes, no key-date pins or caption).
  - **Rows**: Only **active** rows render — rows 1–4 with no visible bar segment and no **in-axis** marker are skipped. Both renderers call **`getActiveTimelineRows()`** (`lib/plan/reportSchedule.ts`) for Timeline-source and **`getCompactPlanTimelineRows()`** for Plan-source. The red **Report date** vertical line height scales to `activeRows.length × rowHeight + 2`.
  - **Month header**: Compact padding (`py-px` in HTML; `paddingVertical: 1` in PDF styles).
  - **Bars**: Clipped to the visible “previous months” range via **`getVisibleBarSegmentsForRow()`** (position/width from the segment within `[timeline.startDate, timeline.endDate]`). Width is natural (`widthPercent(visibleStart, visibleEnd)`) with a **4% cosmetic floor** (`Math.max(rawWidth, 4)`) so very short bars remain visible; `left` stays anchored to the true start date. Bars are fully opaque, rounded (`rounded` / `borderRadius: 2`), with horizontal padding and `overflow-hidden` on the bar container.
  ...
  - **Markers**: Filtered per row with **`getVisibleMarkersForRow()`** — markers outside `[timeline.startDate, timeline.endDate]` are **dropped**, not clamped to an axis edge, so a marker before/after the reported window neither opens a row nor draws at the boundary. HTML preview renders Lucide-equivalent inline SVG from module-level **`TIMELINE_MARKER_ICONS`** (same path/line data as PDF **`TimelineMarkerIconPdf`**). Shapes: BadgeAlert, ThumbsUp, TrendingUpDown, Rocket, PencilRuler, Pin (default Pin).
- **Timeline bars (status report)**: The status report timeline shows only the “previous months” range (e.g. 1–4 months before the report date). Bars are **clipped** to that visible range via `getVisibleBarSegmentsForRow()`: only the segment within `[timeline.startDate, timeline.endDate]` is drawn, and position/width are computed from that segment so the layout matches the shortened axis.
...
### Plan schedule on status reports (`scheduleSource` / `planDensity`)

- **Schedule-eligible variations**: **Standard**, **Milestones**, and **Modular** (`isScheduleEligibleVariation` in `lib/statusReportFlags.ts`). **CDA** is not schedule-eligible. Entering Modular from a non-eligible variation reapplies **`Project.planReportDefault`** for schedule source (`shouldResetScheduleDefaultsOnVariationChange`).
- **Gating**: **`Project.planEnabled`** (**`PATCH`** by User or Admin) controls Plan tab visibility and Plan API access. Per-project flag replaces legacy env allowlists (`lib/plan/feature.ts` → **`isPlanTabEnabled`**). **Plan is off for every project** until an editor enables it.
- **Migrations**: `20260903120000_add_project_plan`, `20260903140000_plan_items_meetings_nesting`, `20260903183710_add_project_plan_beta`, `20260908180000_plan_report_visibility_and_status` (shipped in **1.3.0**), `20260914180000_plan_meeting_unscheduled` in **1.3.3** (`PlanMeetingStatus.assumed` → **`unscheduled`**). `20260903140000` **drops `PlanMeeting`**; that table only existed earlier in this 1.3.0 series (added by `20260903120000`) and was replaced by meeting fields on `PlanItem`, so no prior released deployment can hold data in it.
- **Snapshot fields**: **`StatusReportSnapshot.scheduleSource`** (`timeline` \| `plan`, default **`timeline`** when absent), **`planDensity`** (`phases` \| `phases_and_key_dates`, only when source is **`plan`**), **`timelineLookaheadMonths`** (Plan compact strip, 1–4, default **2**), **`includeDetailedPlan`**, and optional **`timelineLayout`** (per-report hide/rename/row overlay keyed by Plan `phaseId` / `itemId`, plus optional **`windowStartYmd` / `windowEndYmd`**). **`resolveScheduleSource()`** / **`resolvePlanDensity()`** in `lib/statusReportPdfData.ts`. Plan edits never mutate saved snapshots; only create and **Refresh schedule** rebuild `snapshot.timeline`.
- **Compact mapping**: When **`scheduleSource === "plan"`**, **`lib/plan/reportSchedule.ts`** (`compactPlanToSchedule`) maps **`ProjectPlan`** phases and dated items into the existing **`snapshot.timeline`** bars/markers shape, stamping **`phaseId` / `itemId`**, honoring **`showOnReports`**, using Plan names (not a Plan-level short label), and marking completed items **`muted`**. **`lanePolicy`**: **`wrap4`** (default; **Standard** / **Milestones** — phases wrap onto four swimlanes via `order % 4`) vs **`onePhasePerRow`** (**Modular** Plan — one phase per row, rows 1–**`TIMELINE_FILL_ROW_MAX`** (16)). **`resolveReportTimelineAxis()`** windows Plan-source to kickoff ∩ lookback/lookahead (not full `Project.startDate`). **`applyTimelineLayout()`** / **`pruneTimelineLayout()`** in `lib/statusReportTimelineLayout.ts` apply per-report overlays at PDF-data read time (`applyTimelineLayoutOverlay`, default true; refresh persist passes false then prunes overlay ids). **`setTimelineLayoutRow()`** / **`applyTimelineLayout()`** accept **`maxRow`** (default **4**; Modular Plan uses **`timelineLayoutMaxRow("Modular")`** = 16). **`lib/statusReportScheduleBuild.ts`** orchestrates Timeline vs Plan source on create/refresh.
- **Render gate**: **`timelineHasVisibleSchedule()`** / **`getActiveTimelineRows()`** (`lib/plan/reportSchedule.ts`) decide whether a timeline draws anything on rows 1–4 within the axis, and are what both **`TimelineBlock`** renderers use for rows, bar segments, and markers. **`isValidPlanTimeline()`** (`lib/statusReportScheduleBuild.ts`) is the Plan-source create/refresh gate built on it; marker-only Plan key-date schedules pass.
- **API**: **`GET .../status-reports?previousFor=`** returns the latest report on or before that date (including **`snapshot`** and **`panels`**) so a new form can copy narrative, Plan Arrange overlay, and Modular layout. **`POST .../status-reports`** — optional **`scheduleSource`** / **`planDensity`** / **`timelinePreviousMonths`** / **`timelineLookaheadMonths`** / **`includeDetailedPlan`** / **`timelineLayout`** for **Standard** / **Milestones** / **Modular** when Plan is enabled (`isPlanScheduleCreateRequest`); create prunes **`timelineLayout`** onto the new snapshot (`persistTimelineLayoutOnSnapshot`); **400** if Plan source selected but no renderable schedule (`lib/plan/reportScheduleErrors.ts`). **Modular** create locks compact **`snapshot.timeline`** when Plan source is selected even if no **`ganttTimeline`** module is placed yet (`shouldLockModularTimelineOnCreate`); schedule source / density / window / **`includeDetailedPlan`** are still persisted so a later module add + refresh works. **`POST .../refresh-timeline`** rebuilds from Timeline **or** Plan per locked snapshot source; does not change **`scheduleSource`** / **`planDensity`**; prunes **`timelineLayout`** to ids that still exist; on **Modular**, still requires a placed **`ganttTimeline`** module (`modularNeedsTimeline`); returns **400** with **`PLAN_NOT_ENABLED_ERROR`** when the locked source is **`plan`** but Plan has since been disabled, otherwise **`resolveScheduleRebuildError()`** copy (missing project end date vs empty schedule, density-aware for Plan). **`PATCH .../status-reports/[reportId]`** accepts **`timelineLayout`** (including window) and **`includeDetailedPlan`**.
- **UI**: `components/StatusReportsTab.tsx` — schedule source + plan density + Plan lookback/lookahead + detailed-plan checkbox on **create** for all schedule-eligible variations; **Modular** + Plan source uses checkbox label **Add full project plan to report** (`includeDetailedPlan`; same snapshot flag as Standard’s “Include detailed plan page”). When set, preview/PDF append **`PlanPrintDocument`** (`chartOnly`) landscape pages after 16:9 slide(s), before meeting notes. New-report form copies the previous Plan **`timelineLayout`** (hide/rename/rows/window), Plan source/density/window months, and Modular panels when the previous report used them (`timelineLayoutFromPreviousSnapshot`). **Arrange this report’s schedule** (`components/ArrangeScheduleFields.tsx`) on Plan-source **edit**: compact two-column list (phase + wrapping key-date chips with Plan type, date, rename, eye; up/down on the phase). Type comes from compact marker `shape` (`reportKeyDateKind`: milestone / sign-off / hard deadline / meeting). Months shown sits on the Arrange header. `maxRow` still 4 on Standard/Milestones and 16 on Modular. Same `timelineLayout` overlay; no mini Gantt. PATCH saves Arrange on the current report; the next create POST re-applies that overlay as the starting point. Dates stay on Plan. **Refresh timeline** vs **Refresh schedule**. Plan grid View (Compact, 55% cap; status as Lucide `Circle` / `CircleDot` / `CircleCheck`, meetings **Unscheduled** as `Clock`; no left-pane horizontal scroll) vs Edit (full column width, no left-pane horizontal scroll) vs **Chart** (expand rail only, labeled Gantt) vs **Present** (`layoutMode` on `PlanGridGantt`: taller rows, labeled bars, viewport-height pane, optional collapsed left columns via `lib/plan/planGridLayout.ts`). A plan with **no phases** opens in **Edit**. Display rows come from **`buildPlanDisplayRows()`** (`lib/plan/tree.ts`): **Add phase** at the bottom, **Add item** after each expanded phase (including empty phases). Gantt axis pad + today line (`todayMarkerPercent`), **Rpt** / **Status**, item drag (nest / before / onto phase), and **phase drag** (before another phase or onto **Add phase**) in `components/plan/PlanGridGantt.tsx`. Slide **row geometry** for Plan vs Timeline is **`getStatusReportTimelineMetrics()`** (see *Timeline on status reports*); Plan-source reports use named lanes (100px labels, 18px rows). Tests: `__tests__/lib/statusReportFlags.test.ts`, `__tests__/lib/statusReportPdfData.test.ts`, `__tests__/lib/statusReportScheduleBuild.test.ts`, `__tests__/lib/plan/reportSchedule.test.ts`, `__tests__/lib/statusReportTimelineLayout.test.ts`, `__tests__/lib/plan/itemDrop.test.ts`, `__tests__/lib/plan/phaseDrop.test.ts`, `__tests__/lib/plan/positioning.test.ts`, `__tests__/lib/plan/tree.test.ts`, `__tests__/lib/plan/dateInput.test.ts`, `__tests__/lib/plan/completion.test.ts`, `__tests__/lib/plan/ganttBarLabel.test.ts`, `__tests__/lib/plan/planPdfCapture.test.ts`, `__tests__/lib/plan/planGridLayout.test.ts`.

### Refresh budget on saved reports

- **Problem**: Budget (and CDA overall dollars / hours) are locked in **`snapshot.budget`** / **`snapshot.cda`** at create time. Reports created **before** Budget tab lines exist store **`$0` / `0` hours**; later Budget tab or Resourcing actuals edits do not update older reports. **Update** (PATCH) does not refresh the snapshot.
- **API**: **`POST .../status-reports/[reportId]/refresh-budget`** — requires a valid snapshot; rejects **Modular**. Calls **`buildStatusReportPdfData(..., { rebuildBudgetFromProject: true })`**, writes **`snapshot.budget`**, and for **CDA** replaces **`snapshot.cda`** with rebuilt monthly rows / **`totalMtdActuals`** / **`overallBudget`** while **preserving milestones**. Invalidates PDF cache + report tag.
- **Helpers**: **`shouldUseLockedSnapshotBudget()`**, **`shouldRebuildCdaBudgetFromProject()`**, **`applyCdaBudgetRefresh()`** / **`rebuildBudgetFromProject`** in `lib/statusReportPdfData.ts`. **`shouldAttachBudgetToPdfData(variation)`** attaches the budget block for **Standard**, **Milestones**, and **CDA** (Modular omits it).
...
- **Schema**: `StatusReportVariation` includes **`Modular`**. **`StatusReport.panels`** (`Json?`) stores a v1 **`ModularPanelsDocument`**: `{ version: 1, layout: { pages: [{ header: "full"|"compact", rows: [{ id, shape, height, moduleIds }] }] }, modules: Record<id, ReportModule> }`. Legacy `{ type, order, data }[]` still migrates on read. `null`/invalid → Classic default.
- **Normalize**: **`normalizeModularPanels`** (`lib/reportPanels.ts`) is identity for a valid v1 document, migrates the old four-panel array into Classic rows (typed narrative thirds + sprint/story/donuts), and never throws.
- **Types / catalog**: `PANEL_META` — data-owned (`sprintSchedule`, `storyPointMetrics`, `donutKpi`, typed narratives) and feature-linked (timeline, milestones, four budget views, Plan meetings/activities). Unique in layout except **`donutKpi`**.
- **Defaults / presets**: **`MODULAR_DEFAULT_DOCUMENT`** = Classic Modular. **`lib/modularLayoutPresets.ts`** — Classic, Plan delivery, Budget-forward; continuation page (compact header + empty full short row); row/slot edits. Create/update from **`StatusReportsTab`** POST/PATCH **`panels: formModularDoc`** (the live document, not a legacy array).
- **Create snapshot**: Modular always stores **`period`** / **`today`**. When placed modules need them, create also locks **`budget`**, **`timeline`**, and Plan lists (`modularNeedsBudget` / `modularNeedsTimeline` / `modularNeedsPlanLists`). **Plan** schedule source is special: create locks compact **`snapshot.timeline`** via **`shouldLockModularTimelineOnCreate`** even when no **`ganttTimeline`** module is placed yet (so Arrange has bars on first edit); **Refresh timeline** still requires the module (`modularNeedsTimeline`). Timeline-tab source without the module skips locking **`timeline`** (unchanged).
- **Capture**: Modular View uses **`data-slide-width/height` 1440×810**; **`resolveSlideCaptureBox`** pins that box; **`addImage`** still draws onto **720×405 pt** (× `exportScale`). Page 2 captured when **`shouldRenderModularPage2`** (page exists with a non-null module).
- **API**: **`POST`** / **`PATCH`** accept **`panels: z.unknown()`**; persist **`normalizeModularPanels(panels)`**. **`GET`** includes **`panels`** for edit hydration.
- **UI**: Variation **Modular (Story Points / Velocity)**; layout editor (presets, continuation, rows, slots); sprint/story/donut fields driven by modules in the document.
- **Rendering**: Grid in **`StatusReportView.tsx`** and **`StatusReportDocument.tsx`**. **`lib/statusReportPdfData.ts`** passes **`panels`** on **`StatusReportPDFData`**.
- **Refresh Plan lists**: **`POST .../refresh-plan-lists`** when a plan list module is placed; UI **Refresh Plan lists** (`shouldShowRefreshPlanLists`).

---

## Deployment

- **Build** — The build script runs `prisma migrate deploy` then `next build`, so `DATABASE_URL` must be set for the build environment (e.g. Vercel Production and Preview if you deploy there). Pending migrations are applied at build time; seed does not run during build. Releases that add **Slack** tables (`Account`, `AppConfig`, `ResourcingRequest`, `ActualsNudgeLog`, `User.slackUserId`, etc.), **IndustryGroup** (`20260514160928_add_industry_group`), or **Plan** (`20260903120000` / `20260903140000` / `20260903183710` / `20260908180000` / **`20260914180000_plan_meeting_unscheduled`** in **1.3.3**, see *Plan schedule on status reports*) require those migrations to run before the new routes/UI are used.
...
- **Trigger.dev** — Scheduled Float sync and missing-actuals nudges run on Trigger workers, not Vercel. Keep **`runtime: "node-24"`** in `trigger.config.ts`. Redeploy Trigger (`npx trigger.dev@<sdk-version> deploy`, CLI matching `@trigger.dev/sdk`) when worker code or that config changes. You do **not** wait for a Vercel production deploy for a Trigger runtime change to take effect. Details: *Scheduled Float sync (Trigger.dev)* above.
