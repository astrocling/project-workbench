...
| Project | `/api/projects/[id]/plan`, `plan/phases`, `plan/items`, `plan/non-working-days` | Plan tab CRUD (**404** when **`project.planEnabled`** is false). Gated by **`requirePlanSession`** / **`requirePlanEditSession`** (`lib/plan/api.ts`, **`isPlanTabEnabled`**). UI: **`components/PlanTab.tsx`**; tab visibility in **`ProjectDetailTabs.tsx`**. |
...
- **Compact mapping**: When **`scheduleSource === "plan"`**, **`lib/plan/reportSchedule.ts`** (`compactPlanToSchedule`) maps **`ProjectPlan`** phases and dated items into the existing **`snapshot.timeline`** bars/markers shape (same **`TimelineBlock`** renderer). **`lib/statusReportScheduleBuild.ts`** orchestrates Timeline vs Plan source on create/refresh.
- **API**: **`POST .../status-reports`** — optional **`scheduleSource`** / **`planDensity`** for **Standard** / **Milestones** when Plan is enabled; **400** if Plan source selected but no renderable schedule (`lib/plan/reportScheduleErrors.ts`). **`POST .../refresh-timeline`** rebuilds from Timeline **or** Plan per locked snapshot source; does not change **`scheduleSource`** / **`planDensity`**.
