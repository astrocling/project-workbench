...
- **Budget buffer and remaining hours** — `computeBudgetRollups` (`lib/budgetCalculations.ts`) still stores hours-cap leftover as **`remainingAfterProjectedBurnHours*`**. Workbench health uses **dollar** leftover:
  ...
  - **UI:** Budget tab **Overview** card shows spend, hours, and the three rates; **Expected remaining** shows projected burn / remaining $ / remaining hours from dollars / buffer. UI: **`components/BudgetTab.tsx`**. Overview burn donut uses **`burnPercentHighDollars`**. Dashboard **`burnPercent`** is **`burnPercentHighDollars`**. CDA hours-cap math is unchanged.
