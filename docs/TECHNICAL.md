...
- **Timeline on status reports (`TimelineBlock`)**: Implemented in **`components/StatusReportView.tsx`** (HTML preview + client PDF capture) and **`components/pdf/StatusReportDocument.tsx`** (server `@react-pdf/renderer` fallback). **Apply visual changes to both files** so preview, client export, and server PDF stay aligned.
  - **Layout mode**: **`getStatusReportTimelineMetrics(scheduleSource)`** in **`lib/statusReportTimelineLayout.ts`**. **`StatusReportPDFData.scheduleSource`** is set in **`buildStatusReportPdfData`** from **`resolveScheduleSource(snapshot)`** (default **`timeline`** when the snapshot omits it). Both `TimelineBlock`s pass that field through.
    ...
    - **`plan`**: **bands** — **38px** rows (`SR_TIMELINE_ROW_HEIGHT_PX`) split into a **13px** bar band and a marker band below so Plan key-date labels do not cover the next phase bar; markers hang left/right (`timelineMarkerHangsLeft`). Unchanged by the project-timeline overflow fix.
  ...
  - **Pinned layout (PDF)**: `TIMELINE_SLOT_HEIGHT = 70` reserves vertical space for the timeline above the budget block (`BUDGET_BLOCK_RESERVED = 70` unchanged). Worst-case sizing assumes report-date label row + month header + up to four **14px** overlay rows + buffer (Plan-source slides usually have fewer active rows).
  - **Rows**: Only **active** rows render — rows 1–4 with no visible bar segment and no **in-axis** marker are skipped. Both renderers call **`getActiveTimelineRows()`** (`lib/plan/reportSchedule.ts`), the same helper **`timelineHasVisibleSchedule()`** is built on, so the render gate and the rendered rows cannot disagree. The red **Report date** vertical line height scales to `activeRows.length × rowHeight + 2`.
...
- **Snapshot fields**: **`StatusReportSnapshot.scheduleSource`** (`timeline` \| `plan`, default **`timeline`** when absent), **`planDensity`** (`phases` \| `phases_and_key_dates`, only when source is **`plan`**), and optional **`timelineLayout`** (per-report hide/rename/row overlay keyed by Plan `phaseId` / `itemId`). **`resolveScheduleSource()`** / **`resolvePlanDensity()`** in `lib/statusReportPdfData.ts`. Plan edits never mutate saved snapshots; only create and **Refresh schedule** rebuild `snapshot.timeline`.
- **Compact mapping**: When **`scheduleSource === "plan"`**, **`lib/plan/reportSchedule.ts`** (`compactPlanToSchedule`) maps **`ProjectPlan`** phases and dated items into the existing **`snapshot.timeline`** bars/markers shape, stamping **`phaseId` / `itemId`**, honoring **`showOnReports`**, using Plan names (not a Plan-level short label), and marking completed items **`muted`**. **`applyTimelineLayout()`** / **`pruneTimelineLayout()`** in `lib/statusReportTimelineLayout.ts` apply per-report overlays at PDF-data read time (`applyTimelineLayoutOverlay`, default true; refresh persist passes false then prunes overlay ids). **`lib/statusReportScheduleBuild.ts`** orchestrates Timeline vs Plan source on create/refresh.
