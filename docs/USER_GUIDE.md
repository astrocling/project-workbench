...
- **Status reports columns** — **Rpt** chooses whether that phase or item appears on Plan-sourced status-report schedules. Short names for a specific slide are set on that report’s **Arrange this report’s schedule** board, not here. New key dates and scheduled meetings default to shown; tasks, waiting-on-client, and unscheduled meetings default to hidden as markers (they still stretch the phase bar).
...
When **Plan** is enabled for the project (**Settings → Enable Plan tab (beta)**), **Standard** and **Milestones** reports show **Schedule source** when you create a new report:
...
**Modular** reports store the slide grid in **`panels`** as a v1 `{ version, layout, modules }` document (not only sprint/story/donut). The snapshot still holds period/today plus locked **budget**, **timeline**, and **Plan lists** when those modules are placed. Layout and module payloads are editable on **Update**.

- **What you can still edit** on an existing report: narrative fields (completed / upcoming / risks / meeting notes), RAG values and explanations, **variation** where the form allows it, **Show project budget on report** for **Standard** reports (see above), **Arrange this report’s schedule** for Plan-source reports (mini Gantt: drag between rows or Hidden; window start/end; click to rename), **Include detailed plan page**, and the Modular **layout document** (presets, rows, modules, sprint/story/donut payloads).
...
- **Project Plan**: taller rows so phase names sit on a thin bar and **key dates** sit in a band underneath (labels hang left and right of the date). Reports saved with **Project Plan** keep this layout; switching a project’s Plan tab on or off does not change already-saved slides.
- **Bars**: Width reflects the bar’s dates on the axis. Short bars stay narrow; labels **truncate with an ellipsis** (…) when they do not fit, instead of spilling outside the bar. Bar colors match the Timeline tab palette (Plan phase bars may be uncolored).
...
| **Status report timeline covers Completed / Upcoming / Risks** | Fixed in **1.3.1**: **Project timeline** slides use compact 14px rows again. **Project Plan** slides stay taller so key dates sit under phase bars. Hard-refresh preview. |
