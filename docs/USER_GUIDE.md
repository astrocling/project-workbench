...
- **Add phases and items** — In **Edit**, use **+ Add phase** at the bottom of the grid. Under each expanded phase, **+ Add item** creates a top-level task in that phase (even when the phase is empty). Row plus icons still add an item to a phase or after an item. **View** does not show add rows—switch to **Edit** to build the schedule.
