...
| **Timeline** | High-level timeline with month columns and up to four rows of bars and markers. Each bar has a label, start/end dates, row (1–4), and an optional color (Blue, Green, Amber, Teal, Slate, or Violet). On **status report** slides, **Project timeline** is more compact than the Timeline tab (14px overlay rows; only rows that contain bars or markers). **Project Plan** slides put the phase name on the bar. **Condensed** is phases only. **Advanced** uses at most four lines, with key-date pins and names on that line (extra names clip inside the line). Bar labels truncate when space is tight, and markers use the same Lucide-style icons as the Timeline tab. The same timeline (with colors, for Timeline source) appears in status report previews and PDFs. Each saved report stores its **own** copy of the timeline when the report is created; if you change bars or markers later, use **Refresh timeline** or **Refresh schedule** while **editing** that report (**Standard**, **Milestones**, or **Modular** when a timeline module is on the slide) on the Status Reports tab—see [Status Reports tab](#status-reports-tab). |
| **Plan** | (When enabled in **Settings**) Beta schedule builder: a grid plus Gantt with phases and nested items. After **Start from blank**, the grid opens in **Edit** so you can use **+ Add phase** and **+ Add item**. Editors drag **Gantt bars** (or milestone diamonds) to change dates, drag **bar edges** to resize ranges, drag a **phase grip** to reorder phases, and use an **item grip** to reorder, nest, or move items into another phase. **Chart** is a Gantt-only scan (labels on bars); **Present** is a taller labeled layout that hides Plan heading and assumptions and can collapse Name/Dates/Status. **Download PDF** exports a landscape chart (plus assumptions if any). Everyone who can open the project sees the tab. Editors set the **default schedule source** for new **Standard**, **Milestones**, and **Modular** status reports (**Project timeline** or **Project Plan**). The **Timeline** tab remains available; Plan does not replace it. |
| **Status Reports** | Summary table of estimated budget, $ spent, $ remaining, budgeted/actual/remaining hours, with copy-to-clipboard and a % budget used (high est.) circle chart. **New report** and **Edit report** open in a dialog over the list. Choose a **variation**: **Standard** (timeline + budget), **Milestones**, **CDA**, or **Modular** (1440 composer grid: presets, rows, modules)—see [Status Reports tab](#status-reports-tab). When **Plan** is enabled, **Standard**, **Milestones**, and **Modular** reports can choose **Project timeline** or **Project Plan** (**Condensed** or **Advanced**) at create time—see [Schedule source (Plan enabled)](#schedule-source-plan-enabled). **Standard** reports include an optional **Show project budget on report** toggle (default on) to hide the budget table and burn chart on the exported slide. **New** reports cannot be saved while **actuals are stale** (completed weeks with planned hours but missing actuals)—update the Resourcing tab first; the block clears immediately once actuals are filled in. When Slack is configured, **Post to Slack** (Slack icon) sends the latest saved report summary to the linked account's Slack channel. **Standard**, **Milestones**, and **Modular** (when a timeline module is placed) can **refresh the stored timeline** from the Timeline tab or **refresh the stored schedule** from the Plan tab after the source changes. **Standard**, **Milestones**, and **CDA** reports can **Refresh budget** after Budget tab or Resourcing actuals change (on **CDA** that also refreshes locked monthly/overall hours)—see [Refresh budget](#refresh-budget-standard-milestones-and-cda-editors-only). **Update** alone does not change hours on the slide. **CDA** reports can **refresh stored milestone dates** after the CDA tab changes—see [Refresh milestones (CDA)](#refresh-milestones-cda-editors-only). |
| **Settings** | Edit project name, client, dates, status, single rate, notes, SOW/Estimate/Float/Metric links, optional **Slack channel ID** (for missing-actuals Tue/Wed nudges and **Post week to Slack**), resourcing thresholds, key roles, optional **CDA** tab, and **Enable Plan tab (beta)**. Within Settings, sub-sections include **Details** (includes read-only **Industry group** inherited from the linked Float **client account** when `accountId` is set — see Admin → Accounts), **Links**, **Key roles**, **Resourcing** (thresholds), **Rates** (per-role rate card or single bill rate), and **Assignments** (people assigned, their roles, bill-rate overrides, and “hidden from grid” for the Resourcing tab). |
...
|----------------------|---------|
| **Team this week** | Anyone with **Planned** or **Float** hours **> 0** on the current Monday–Sunday **UTC** week (same week as Resourcing). Each line includes that week’s **hours allocation**: **Planned** hours when they have them, otherwise **Float** hours. Hidden-from-grid people still appear if they have hours. Names use Slack **@mentions** when the person is linked to a Workbench user with a **Slack user ID**. **(Not in Plan)** means they have Float hours this week but no Planned hours (Workbench planned grid vs Float—not the Plan tab). |
| **This week** | Plan items (meetings, deadlines, milestones, tasks, and so on) and Timeline markers/bars whose **start date or end date** falls in that week. A long bar that only covers the week does **not** appear unless it starts or ends that week. Each line shows a same-day date, an in-week range, or **starts** / **ends** with the other bound when it falls outside the posted week. Meeting times are included when set. |
...
When **Plan** is enabled for the project, the tab is a Smartsheet-style grid (left) and Gantt (right):

- **Compact, Full, and Chart** — Plans that already have phases open in **View** (Compact): nested names, a type icon, shortened dates (`M/DD`), and a **status icon** (empty circle / dotted circle / check; hover for **Not started**, **In progress**, or **Complete**). Meetings use **Unscheduled** (clock), **Scheduled**, or **Complete**. A phase shows the complete check when **every** item in it is complete. Switch to **Edit** (Full) to see Type, Start, End, Days, **Rpt**, Status, and row actions. **Chart** hides Name, Dates, and Status and leaves a narrow expand rail so phase/parent chevrons still work; bar and diamond labels carry the names. A **blank** plan (no phases yet) opens in **Edit** so **+ Add phase** is visible. Compact is for scanning; change names, types, dates, and status in Edit. Viewers without edit permission get **View** and **Chart**. **Neither View nor Edit scrolls the left table sideways**; the Gantt uses the remaining width.
- **Add phases and items** — In **Edit**, use **+ Add phase** at the bottom of the grid. Under each expanded phase, **+ Add item** creates a top-level task in that phase (even when the phase is empty). New items in an empty phase start on **today** (clamped to the plan window); if the phase already has items, they start on that phase’s earliest date. Row plus icons still add an item to a phase or after an item (after copies that item’s dates). **View** does not show add rows—switch to **Edit** to build the schedule.
...
- **Present and PDF** — **Present** sits with **View** / **Edit** / **Chart** and Gantt zoom. It labels bars and diamonds, uses more of the window height, and hides the Plan heading and Assumptions so the chart fills the tab. **View** while Present keeps Name, Dates, and Status; use **Hide columns** to collapse that pane to the expand rail (or switch to **Chart** for Gantt-only). **Fit** is the default zoom in Present (your Work zoom comes back when you toggle **Present** off, press **Escape**, or click **Edit**). **View** and **Chart** do not exit Present. **Download PDF** (same toolbar) exports a **US Letter landscape** two-page plan: timeline with kickoff/launch/duration, then phase detail and assumptions. It is not a screenshot of the editor grid.
- **Dates on the Gantt** — Drag a task or unscheduled-meeting bar to move it (duration stays the same). Drag the left or right edge to resize. Drag a milestone, sign-off, hard-deadline, or scheduled-meeting diamond to change that single date. Dates snap to calendar days (the same as the Start/End cells). Changing **Start** in the grid keeps the item’s duration (so a later start does not fail because End is still the old date). The **End** picker cannot choose a day before Start and opens on the item’s start. Child rows do not move when you drag a parent bar. **Escape** cancels a drag in progress (in Present it exits Present).
...
- **Status reports columns** — **Rpt** chooses whether that phase or item appears on Plan-sourced status-report schedules. Short names, hide/show, and which **Advanced** line a phase uses are set on that report’s **Arrange this report’s schedule** list, not here. New key dates and scheduled meetings default to shown; tasks, waiting-on-client, and unscheduled meetings default to hidden as markers (they still stretch the phase bar).
- **Status** — Mark items **Not started**, **In progress**, or **Complete**. Meetings use **Unscheduled** (date range), **Scheduled** (single date), or **Complete** (same Complete as other items). **View** uses icons for those states (hover for the full label). The Plan header shows how many items are complete. A phase is marked complete when all of its items are complete. Completed key dates still appear on new reports (when **Rpt** is on) but are drawn faded.
...
- **Plan tab** — **Enable Plan tab (beta)** is on the **Details** section next to **Enable CDA**. Any editor can turn it on for this project. Everyone who can open the project then sees **Plan**. It stays off until someone enables it.
...
- **Sticky HOL / PTO / dates**: One unlabeled strip of **HOL** / **PTO** pills (HOL above **PTO** in the week column when both apply) plus the week-date row (Ready / Person / Role / Total + week labels) sits above all three grids. When weeks overflow, **left / right** buttons sit on that same pill row (left of the pills), not in a separate toolbar. As you **scroll the grid pane down**, that header **stays at the top** while Planned, Actuals, and Float slide underneath. Each grid still has its own title and buttons (Request Changes, Actuals chevron, Sync plan from Float); titles are not in a gray boxed cell.
- **Week columns stay aligned**: Horizontal week motion is **one scroller** for the header and all three tables. If Planning is on the week of Sep 7, Actuals and Float (and the HOL/PTO pills) are on that same week. Use trackpad, the pane scrollbar, **Shift + mouse wheel**, or the **left / right** buttons beside the pills when weeks overflow. **Load earlier / later weeks** still sits above the pane.
...
- **Current week**: The column for the **Monday–Sunday week (UTC) that contains today** has a **subtle background tint** from the **week-date** header down through all three grids (Planned, Actual, Float) and the variance/total rows. The **HOL / PTO** pills are **not** tinted. (Amber variance or red mismatch highlights still take priority when they apply.)
...
- **PTO and holidays**: When **Admin → Float sync** has run, the **shared header** shows **HOL** and **PTO** pills in each week column (stacked when both apply), and planned-hour cells can show PTO/holiday borders. Hover a pill or a marked cell for details. The hover card **sizes to the text** (long names are not clipped) and opens **below** the trigger in a floating card so it stays readable while the header is stuck.
  ...
  - **Float grid vs PTO**: **Full-day** PTO and **holidays** drop that person from Float hours on those weekdays. **Partial PTO** does **not**—if Float still has project hours that day (for example 0.5h alongside 4h PTO), the **Float** column includes them. After deploying **1.3.2**, run **Admin → Float sync** so stored weeks refresh.
...
| **Modular (Story Points / Velocity)** | 1440×810 composer grid (presets, rows, modules, optional continuation slide)—see [Modular variation](#modular-variation-story-points--velocity) below. |

**Copy from previous report** still pre-fills narrative and RAG from the most recent report before the selected date, regardless of variation. When that previous report used **Project Plan**, the new form also starts from its schedule source, density, month window, and **Arrange** overlay (hide, short names, line, and label positions). A previous **Modular** layout is copied the same way.

### Schedule source (Plan enabled)

When **Plan** is enabled for the project (**Settings → Enable Plan tab (beta)**), **Standard**, **Milestones**, and **Modular** reports show **Schedule source** when you create a new report:
...
| **Plan density** | Shown when source is **Project Plan**: **Condensed** (phases only) or **Advanced** (phases + key dates; default). Fine-grained show/hide lives on the **Plan** tab (**Rpt**), then freezes into the report snapshot at create time. Short names for that slide are edited on **Arrange this report’s schedule**. **Advanced** uses at most **four** lines (later phases wrap back across those lines). The phase name sits on the bar. Each key date is a pin on that line, with its name on a short leader from the bottom of the pin. Names that do not fit **clip** inside the line. |
...
| **Include detailed plan page** | Plan-source: optional extra PDF/preview page with the full Plan Gantt (kickoff to launch). The 16:9 slide stays a compact strip. On **Modular** the checkbox label is **Add full project plan to report**. |

**CDA** does not show these fields. Projects with Plan **off** always use **Project timeline** (legacy behavior).

On **edit**, schedule source, plan density, previous months, and Plan lookahead are **locked** (read-only), same as report date. Editing the Plan tab does **not** change saved reports. Use **Refresh timeline** or **Refresh schedule** (below) to replace the stored bars/markers after changing the Timeline or Plan tab. For Plan-source reports you can also **Arrange this report’s schedule**: a **Phase** / **Key dates** list, plus a live chart when density is **Advanced**.

- Rename a phase or key date in its text field (this slide only).
- The eye hides or shows that phase or key date.
- Up/down moves a phase. **1–4** places that phase on a line. **Condensed Modular** can use more than four rows and does not show **1–4**.
- Left/right on a key date moves it to the neighboring phase. Dates stay on the Plan tab.
- **Months** on the Arrange header narrows the strip. It cannot reopen the full project year.
- On the Advanced chart, drag a phase bar up or down between the four lines. Dropping below the last occupied line opens the next empty line (up to 4). Drag a key-date **name** into a quieter month; the pin stays on the real date.

**Include detailed plan page** can be turned on or off while editing. If Plan is later disabled on the project, reports that used **Project Plan** still show their locked schedule source and density when you edit them, and the saved schedule keeps showing on the slide — but **Refresh schedule** fails with *“Plan is not enabled for this project.”* until an editor turns Plan back on in **Settings**.

### Modular variation (Story Points / Velocity)

Use **Modular** when the slide should be a **fill-the-slot grid** instead of the Standard budget block. The composer canvas is **1440×810** (16:9). Preview and PDF keep the same 16:9 page; capture scales that canvas onto the 720×405 pt slide.

When you select **Modular (Story Points / Velocity)**, the form stores a **layout document** (`layout` + `modules`) on the report. **Save** / **Update** persist that document (not the old four-panel array). You can:

1. **Apply a preset** (replaces the grid; sprint / story-point / donut numbers are kept when the new preset still has those module types):
   - **Classic Modular** — page 1 full header; tall thirds of **Completed** / **Upcoming** (typed) / **Risks**; halves sprint | story points; halves two donut KPIs. New Modular reports start here, with typed narrative columns (the top-of-slide completed / upcoming / risks fields still feed those modules).
   - **Plan delivery** — Plan **Completed** / **Upcoming** activities plus typed **Risks**; **Upcoming Meetings** on a full row (or halves with compact dollar budget when Plan is on). Greyed out when Plan is off.
   - **Budget-forward** — typed narrative thirds plus a full **Budget & Financials** table.
2. **Add continuation slide** — one extra page with a **compact** header and an empty full short row. Hidden once page 2 exists (two module pages max). The second PDF page is omitted until that slide has at least one module.
3. **Add slide row** — pick shape (**Full**, **Halves**, **Thirds**, **Wide + KPI**, **KPI + Wide**) and height (**Tall** / **Short**), appended to the selected page.
4. **Per row:** move up/down or remove. **Per slot:** add a module (most types only once; **Donut KPI** may repeat) or remove it (slot becomes empty). If **Plan** is off, Plan activity and meetings modules are greyed out; modules already on the report stay.

**Module catalog**

| Module | What it shows |
|--------|----------------|
| Completed / Upcoming (typed) | The report’s typed **Completed activities** / **Upcoming activities** fields. |
| Risks / Issues / Decisions | The typed **Risks** field (never a Plan list). |
| Completed / Upcoming (from Plan) | Locked Plan activity lists from the snapshot. |
| Upcoming Meetings | Unscheduled vs scheduled Plan meetings. |
| Sprint Schedule | Date-range + label rows (Add row / Remove). Empty rows are omitted on the slide. |
| Story Points | Up to **four systems**; Planned / Completed / In Progress / Carry Over. |
| Donut KPI | Label + manual 0–100%. |
| Timeline / Milestones | Feature-linked schedule strip or milestone table when placed. |
| Budget & Financials | HIGH/LOW $ and hours table + HIGH $ burn donut. |
| Budget (compact dollars) | HIGH $ Est / Spent / Remaining + donut. |
| Budget (compact hours) | HIGH hours Budgeted / Actual / Remaining + hours donut. |
| Budget burn only | Donut + “Budget used”. |

When **Plan** is on, **Modular** create uses the same **Schedule source** fields as Standard. Choosing **Project Plan** stores the compact schedule immediately, even if the grid does not have a timeline module yet, so **Arrange** has bars on the first edit. **Refresh timeline** still appears only after a timeline module is placed.

**Refresh** on an existing Modular report (editors): **Refresh budget** when a budget module is placed; **Refresh timeline** when a timeline module is placed; **Refresh Plan lists** when a Plan meetings/activities module is placed. Those buttons rewrite the snapshot, not the layout.

Sprint, story-point, and donut **inputs appear only when those modules are on the grid**.

New Classic reports start with empty schedule rows and story-point systems; donut KPIs default to **0%**.

Modular reports still use the same biographical block and RAG table. They **do not** use the Standard bottom budget block unless you place a budget module.
...
This is a different action from Overview **Post week to Slack**, which posts a current-week team (with hours) and Plan/Timeline dates digest to the **project** channel.
...
**New report** and **Edit report** open in a dialog over the reports list (the page behind it does not scroll). **Close** (while editing), **Cancel** (while creating), or **Escape** dismisses the dialog. If a refresh confirmation is open, **Escape** closes that confirmation first. **View** in the edit dialog, and **Preview** on a list row, open the slide preview on top of the dialog when it is open.

When you click **Save** (new report) or **Update** (while editing), Workbench saves the report, **closes the dialog**, refreshes the list, and **scrolls** the page so the **Status Reports** heading and table are in view. That makes it obvious the save succeeded and reduces the chance of clicking save again and creating a duplicate. For a **new** report, the list jumps to **page 1** (reports are ordered with the newest dates first, so your new row is usually there). Use **Preview** (eye icon) on a row when you want to open the report after saving.
...
**Modular** reports store the slide grid in **`panels`** as a v1 `{ version, layout, modules }` document (not only sprint/story/donut). The snapshot still holds period/today plus locked **budget**, **timeline**, and **Plan lists** when those modules are placed. A **Project Plan** source also locks the compact timeline on create even when no timeline module is placed yet. Layout and module payloads are editable on **Update**.

- **What you can still edit** on an existing report: narrative fields (completed / upcoming / risks / meeting notes), RAG values and explanations, **variation** where the form allows it, **Show project budget on report** for **Standard** reports (see above), **Arrange this report’s schedule** for Plan-source reports (phase and key-date list: rename, hide, line **1–4**, move a key date to another phase, month window; on Advanced, drag bars between lines and drag key-date names), **Include detailed plan page**, and the Modular **layout document** (presets, rows, modules, sprint/story/donut payloads).
...
### Refresh timeline or schedule (Standard, Milestones, and Modular, editors only)

If you updated the project **Timeline** tab **after** a report was saved with **Project timeline**, the report still shows the **old** timeline until you refresh it. If you updated the **Plan** tab after a report was saved with **Project Plan**, use **Refresh schedule** instead. On **Modular**, the button is available when a timeline module is on the slide.
...
3. Read the confirmation dialog: it explains that the **schedule stored on this report** will be **replaced** with your project’s **current** Timeline bars/markers or **current** Plan phases/key dates (including current **Rpt** settings). Per-report arrange tweaks (line, hide, short names, key-date label positions) are kept when the same phase or item still exists. Your **report date** and **how many previous months** are shown do **not** change; other snapshot data (for example budget) is **not** affected.
...
2. In the edit dialog, click **Refresh milestones on report**.
...
When a Standard or Milestones report includes a timeline (project has bars in the snapshot window), the slide shows a strip above the budget section (or above modular panels when applicable):

- **Month columns** align to weeks in the reporting window (same logic as the Timeline tab).
- **Rows**: The slide **only shows rows that have at least one bar or marker** — blank rows are omitted. **Project timeline** and **Advanced** Plan use at most four lines. **Condensed Modular** can use more rows when many phases are shown.
- **Project timeline** (default, and the only option when Plan is off): compact **14px** rows with full-height colored bars and markers sitting on the bar. This is the layout that fits four rows on the 16:9 slide without covering Completed / Upcoming / Risks.
- **Project Plan, Condensed**: phase name inside the colored bar. No key-date pins.
- **Project Plan, Advanced**: up to four lines, phase name on the bar, key-date pin on that line with the name beside it. Clustered names stack downward and clip inside the line. Reports saved with **Project Plan** keep this layout; switching a project’s Plan tab on or off does not change already-saved slides.
- **Bars**: Width reflects the bar’s dates on the axis. Short bars stay narrow; labels **truncate with an ellipsis** (…) when they do not fit, instead of spilling outside the bar. Bar colors match the Timeline tab palette (Plan phase bars use the phase color).
...
Preview, **Download PDF**, and the server PDF fallback all use the same timeline rendering for that report’s schedule source.
...
Admins pull scheduled hours from the **Float API** (no file upload). The app reads Float people, projects, clients, roles, tasks, **time off**, **public holidays**, and **team holidays** for the same date window and updates Workbench to match. Scheduled hours use **UTC weekdays** in that window, minus **per-person** non-working days: **full-day** Float **time off** (missing hours, `full_day`, or 8h+ per day), plus **public** and **team** holidays **only when** the person’s **Float region** matches the holiday’s region (see Admin People **Region** below). **Partial PTO** (for example 4h) does **not** zero remaining hours Float still has on that project that day. People **without** a Float region still get **full-day time-off** exclusions but **not** regional holiday exclusions—set regions in Float if holiday weeks should line up. Time off is matched to people the same way for **scheduled-hour totals** and for **PTO** features (Float may attach time off to **`people_ids`** rather than a single **`people_id`** field).
...
- **Partial PTO used to zero Float (fixed in 1.3.2):** If someone has **partial** time off and remaining hours in Float, older builds stored **0** for that week. After **1.3.2**, run **Admin → Float sync** and hard-refresh **Resourcing**.
...
| **Float shows hours but Workbench Float is 0 (partial PTO week)** | Fixed in **1.3.2**: **partial PTO** no longer zeros remaining Float task hours. **Full-day** PTO and **holidays** still drop those weekdays. Redeploy, run **Admin → Float sync**, hard-refresh **Resourcing**. |
...
| **Refresh schedule says “Plan is not enabled for this project.”** | The report was created from **Project Plan**, but Plan has since been turned off in **Settings**. The saved schedule still shows on the slide; turn **Enable Plan tab (beta)** back on before refreshing. |
| **Cannot drag phases or items on Plan** | You need **edit** permission, and Plan must be enabled. Viewers see the grid and Gantt without grips. Drag from the **grip**, not the name. Use a **phase** grip to reorder phases; an **item** grip to nest or move items. If the drop highlight disappears, you are over a no-op target (same position). **Present** and **Chart** are read-only — click **Edit** (exits Present) to change the grid. |
| **Plan PDF is missing, huge, or unreadable** | **Download PDF** builds a **letter landscape** two-page document (timeline, then phase detail). It is not a screenshot of the grid. Stay on the Plan tab and try again; there is no server fallback. |
| **Blank Plan has no way to add an item** | Fixed in **1.3.1**: a plan with no phases opens in **Edit** with **+ Add phase**. After you add a phase, **+ Add item** sits under that phase. If you only see names and the Gantt, click **Edit**. |
| **Plan Start date does not stick when I pick End** | Fixed in **1.3.3**: Start used to fail if it was after the current End (typical on new items). Changing Start now keeps duration so End moves with it. Redeploy **1.3.3**. |
| **Plan date picker always opens in January** | New items used to start on kickoff (often Jan 1). In **1.3.3**, items in an empty phase start on **today** (clamped to the plan); otherwise they start on the phase’s earliest date. The End picker cannot choose a day before Start. |
| **Status report timeline covers Completed / Upcoming / Risks** | **Project timeline** slides use compact 14px rows. **Advanced** Plan keeps key-date names on the phase line (they clip inside the line instead of opening a band above the months). Hard-refresh preview. |
...
| **Weekly Slack post is missing people or dates** | People need **Planned** or **Float** hours **this** UTC week; each person line shows **Planned** hours when they have them, otherwise **Float**. Dates come from the **Plan** tab and **Timeline** tab only if the item **starts or ends** this week (not merely spans it). Lines qualify the date as same-day, an in-week range, or **starts** / **ends**. **(Not in Plan)** means Float hours without Planned hours. |
...
| **Resourcing week columns don’t line up across Planned / Actuals / Float** | All three grids and the HOL/PTO header share one scroller. Hard-refresh. Scroll weeks with **Shift + mouse wheel** or the **left / right** buttons on the pill row, not a separate scrollbar on each table. |
