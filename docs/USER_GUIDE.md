...
| **Float hours look low vs Float** | If someone has **multiple allocations** on the same project (workstreams, or split days in a week), older syncs kept only the largest block per day. After this fix, run **Admin → Float sync** and hard-refresh **Resourcing**. |
