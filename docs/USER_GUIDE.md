...
- **Compact and Full** — The grid opens in **Compact**: nested names, a type icon, shortened dates (`M/DD`), and a shortened status. Switch to **Full** to see and edit Type, Start, End, Days, **Rpt**, and Status. Compact is for scanning; change names, types, dates, and status in Full. If Full columns need more room than the pane, the grid scrolls horizontally so labels are not clipped.
