...
| **Plan** | (When enabled by an **Admin** in **Settings**) Beta schedule builder with phases and key dates. Everyone who can open the project sees the tab. Editors set the **default schedule source** for new status reports (**Project timeline** or **Project Plan**). The **Timeline** tab remains available; Plan does not replace it. |
