...
| **Resourcing** | Planned hours, actual hours, and Float scheduled hours by person and week. Use this to compare plan vs actual vs Float and spot gaps. You can **collapse Weekly Actuals** (chevron on that section) to bring Planned and Float closer on screen. When Slack is configured, use **Request Resourcing Changes** to notify the org resourcing channel (see [Resourcing tab](#resourcing-tab)). The **Project Planning** grid shows **PTO** and **holiday** pills per week when Float sync has populated time off and holidays; hover for names, days, and weekday letters (see [Resourcing tab](#resourcing-tab)). |
...
- **Planning grid — PTO and holidays**: When **Admin → Float sync** has run, the **Project Planning** grid shows a **Holiday** row (**HOL** pills) and a **PTO** row (**PTO** pills) above the person hours, and planned-hour cells can show PTO/holiday borders. Hover a pill or a marked cell for details. The hover card **sizes to the text** (long names are not clipped) and opens **below** the pill so it is not hidden under the row above.
...
| **PTO/holiday hover is cut off or sits under the row above** | Current builds size the hover card to the text and open it **below** the pill. Hard-refresh the **Resourcing** tab. Weekday letters are **M T W R F** (**R** = Thursday). |
