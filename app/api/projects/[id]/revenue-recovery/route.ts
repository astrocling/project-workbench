...
const getCachedRevenueRecovery = unstable_cache(
  async (id: string): Promise<RevenueRecoveryResponse | null> => {
    ...
    for (const monthKey of monthKeys) {
      const { forecastDollars: fd, actualDollars: ad } = monthTotals.get(
        monthKey
      )!;
      ...
      const recoveryPercent = fd > 0 ? (ad / fd) * 100 : null;
      const overallRecoveryPercent =
        monthComplete && cumulativeForecastToDate > 0
...
export async function GET(
  ...
  const data = await getCachedRevenueRecovery(id);
