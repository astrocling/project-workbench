...
import {
  getCachedPortfolioMetricsForPgm,
...
const DASHBOARD_SORT_KEYS = ["name", "clientName", "burnPercent", "bufferPercent", "recovery4WeekPercent", "actualsStatus"] as const;
...
export default async function PGMDashboardPage({
  searchParams,
}: {
  ...
  const metricsNoFilter =
    currentPersonId != null
      ? await getCachedPortfolioMetricsForPgm(currentPersonId).catch(() => emptyMetrics)
      : emptyMetrics;
  ...
  if (clientParam != null && clientParam !== "" && validClient == null) {
    redirect("/pgm-dashboard");
  }
  const portfolioMetrics =
    currentPersonId != null && validClient
      ? await getCachedPortfolioMetricsForPgm(currentPersonId, validClient).catch(
          () => metricsNoFilter
        )
      : metricsNoFilter;
  ...
  return (
    <div>
      ...
      <p className="text-body-md text-surface-600 dark:text-surface-300 mb-6">
        Metrics for projects where you are PGM (Program Manager).{" "}
        <Link
          href="/projects"
